...
$filename = $this->state->get('shoprule.title', 'shop-slabs');
...
$rows   = array();
$rows[] = array('Min', 'Max', 'Origin_Country', 'Origin_State', 'Origin_Zip', 'Delivery_Country', 'Delivery_State', 'Delivery_Zip', 'Amount', 'Unit');
...
foreach ($slabs as $i => $record)
{
	...
	$amntUnit  = ArrayHelper::getValue($record, 'unit', '', 'string');
	...
	$rows[] = array($min, $max, $o_country, $o_state, $o_zip, $country, $state, $zip, $price, $amntUnit);
}
