...
class CategoryProcessor extends AbstractProcessor
{
	...
	protected function extractCategoryNames($obj)
	{
		...
		if (isset($obj->product_categories))
		{
	...
	protected function extractCategories($obj, $fields = array())
	{
		...
		if ($obj->x__category_titles)
		{
			...
			$canCreate = ($duplicate || $scope == 'global') && $this->importer->getOption('create.categories', 0);
			...
			foreach ($catNames as $catName)
			{
				try
				{
					...
					$catId = Category::getId($catName, $type, $canCreate, array_keys($fields), $category);
				}
				catch (\Exception $e)
				{
				...
				if ($catId)
				{
					$cat         = $this->helper->category->getItem($catId);
					$params      = new Registry($cat->params);
					$uniqueField = $params->get('product_unique_field', '');

					// If the importer scope is global, the category should use global unique field
					if ($scope == 'product_category' || $uniqueField == '')
					{
						$categories[] = $catId;
					}
				}
