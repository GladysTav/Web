...
class PlgSystemSellaciousImporter extends SellaciousPluginImporter
{
	...
	public function onCollectHandlers($context, array &$handlers)
	{
		if ($context == 'com_importer.import')
		{
			...
			if (!$this->handlers && $this->helper->access->check('importer.import', null, 'com_importer', $userId))
			{
				if ($this->helper->access->check('product.create', null, 'com_sellacious', $userId) || $this->helper->product->canEdit())
				{
					$this->handlers['products'] = new ImportHandler('products', JText::_('PLG_SYSTEM_SELLACIOUSIMPORTER_API_IMPORT_PRODUCTS'));
				}

				if ($this->helper->access->checkAny(array('create', 'edit', 'edit.own'), 'category.', '', 'com_sellacious', $userId))
				{
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		if ($form instanceof JForm)
		{
			...
			if ($name == 'com_importer.template' && isset($subject->import_type))
			{
				...
				if ($subject->import_type == 'images')
				{
					$form->removeField('mapping_note');
					$form->removeField('mapping');
				}
				elseif ($subject->import_type == 'products')
				{
					if (!$this->helper->access->checkAll(array('create', 'edit'), 'user.'))
					{
						if (!$this->helper->access->check('user.create'))
						{
							$form->removeField('sellers', 'params.create');
						}

						if (!$this->helper->access->check('user.edit'))
						{
							$form->removeField('sellers', 'params.update');
						}

						if (!$this->helper->access->checkAny(array('create', 'edit'), 'user.'))
						{
							$form->setFieldAttribute('seller', 'type', 'hidden', 'params.unique_key');
							$form->setFieldAttribute('seller', 'class', '', 'params.unique_key');
						}
					}

					if (!$this->helper->access->check('variant.create'))
					{
						$form->setFieldAttribute('variants', 'type', 'hidden', 'params.create');
					}

					if (!$this->helper->access->checkAny(array('edit', 'edit.own'), 'variant.'))
					{
						$form->setFieldAttribute('variants', 'type', 'hidden', 'params.update');
					}

					if (!$this->helper->access->checkAny(array('create', 'edit', 'edit.own'), 'variant.'))
					{
						$form->setFieldAttribute('variant', 'type', 'hidden', 'params.unique_key');
						$form->setFieldAttribute('variant', 'class', '', 'params.unique_key');
						$form->setFieldAttribute('variant_independent', 'type', 'hidden', 'params');
					}

					if (!$this->helper->access->check('user.create'))
					{
						$form->removeField('manufacturers', 'params.create');
					}

					if (!$this->helper->access->check('user.edit'))
					{
						$form->removeField('manufacturers', 'params.update');
					}

					if (!$this->helper->access->check('category.create'))
					{
						$form->removeField('categories', 'params.create');
					}

					if (!$this->helper->access->check('splcategory.create'))
					{
						$form->removeField('special_categories', 'params.create');
					}

					if ($this->helper->config->get('allow_duplicate_products'))
					{
						$form->removeField('product_scope', 'params');
						$form->removeField('categories', 'params');
					}
					elseif ($registry->get('params.product_scope') == 'product_category')
					{
						$form->removeField('categories', 'params.create');
						$form->setFieldAttribute('product_category', 'required', true, 'params');
					}
				}
			}
		}
	...
	public function onContentPrepareData($context, $data)
	{
		...
		if ($context == 'com_sellacious.template')
		{
			$user = JFactory::getUser();

			if (is_object($data) && !$this->helper->access->checkAll(array('create', 'edit'), 'user.', '', 'com_sellacious', $user->id))
			{
				$params = new Registry($data->params);
				$params->set('unique_key.seller', 'seller_username');
	...
	{
		JForm::addFieldPath(__DIR__ . '/forms/fields');
		$form = parent::getConfigForm($handler, $template);

		$registry = new Registry($template);

		if ($form && isset($template->override) && (int) $template->override === 1)
		{
			if (!$this->helper->access->checkAll(array('create', 'edit'), 'user.'))
			{
				if (!$this->helper->access->check('user.create'))
			...
			if (!$this->helper->config->get('allow_duplicate_products') && $registry->get('params.product_scope') == 'product_category')
			{
				$form->removeField('categories', 'params.create');
			}

			$form->setFieldAttribute('product_scope', 'type', 'hidden', 'params');
			$form->setFieldAttribute('product_category', 'type', 'hidden', 'params');
		}

		if ($handler == 'products' || $handler == 'categories')
		{
			...
			if (!$form)
			{
				...
				JForm::addFieldPath(JPATH_SELLACIOUS . '/components/com_importer/models/fields');

				$form = JForm::getInstance($this->pluginName . '.import.' . $handler, 'other_config');
			}
			else
			{
				$form->loadFile(__DIR__ . '/forms/other_config.xml');
			}

			$form->setValue('file_encoding', 'params', $import->options->get('file_encoding'));
		}
	...
	public function onImportSetOptions($context)
	{
		...
		if ($import->handler === 'products')
		{
			...
			$options = $template ? $template->params : array();
			...
			if (!isset($options['file_encoding']))
			{
				$params = $this->app->input->get('params', array(), 'array');

				$options['file_encoding'] = $params['file_encoding'];
			}
	 ...
	 * @since   1.5.0
	 */
	public function onRequestExport($context, $filename, $templateId = 0, $filters = null)
	{
		if (!$filename)
		{
			return;
	...
	protected function setupCsvImport($handler)
	{
		...
		$encoding = mb_detect_encoding(file_get_contents($path), mb_list_encodings());

		$params['file_encoding'] = $encoding;
