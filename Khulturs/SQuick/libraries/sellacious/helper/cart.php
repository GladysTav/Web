...
class SellaciousHelperCart extends SellaciousHelperBase
{
	 ...
	 * @since   1.0.0
	 */
	public function getCart($userId = null, $options = array())
	{
		return Sellacious\Cart::getInstance($userId, $options);
	}

	/**
	 * Method to round off price total
	 *
	 * @param    float  $total  The total price
	 *
	 * @return   float|int
	 *
	 * @since    1.7.0
	 */
	public function getRoundedTotal($total)
	{
		$nearest   = $this->helper->config->get('grand_total_rounding_number', 1.00);
		$direction = $this->helper->config->get('grand_total_rounding_direction', 1);

		if ($direction == 1)
		{
			$total = ceil($total / $nearest) * $nearest;
		}
		elseif ($direction == 2)
		{
			$total = floor($total / $nearest) * $nearest;
		}
	...
	public function makeOrder($user_id = null)
	{
		...
		$shippingInProduct = $cartOptions->get('product_select_shipping', 0);

		$status_change    = $this->helper->config->get('order_status_change', 0);
		$seller_recursive = $status_change == 1;
		...
		foreach ($items as $index => $item)
		{
			...
			$product->shipping_taxes       = $item->getShipping('tax_amount');
			$product->shipping_discounts   = $item->getShipping('discount_amount');
			$product->shipping_subtotal    = $item->getShipping('sub_total');
			...
			$product->shipping_shoprules   = $item->getShipping('shippingShopRules');
		...
		$oNumber = $this->helper->order->genOrderNumber($userId, $hash);
		...
		$order->product_shipping_taxes     = $totals->get('shipping_tax_amount');
		$order->product_shipping_discounts = $totals->get('shipping_discount_amount');
		$order->product_shipping_subtotal  = $totals->get('shipping_subtotal');
		...
		$order->shipping_shoprules         = $totals->get('shipping_shoprules');
		$order->shipping_params            = json_encode($itemisedShip == SellaciousHelperShipping::SHIPPING_SELECTION_PRODUCT ? $shippingForm : $cart->getParam('shippingformdata'));
		$order->checkout_forms             = json_encode($cart->getParam('checkoutformdata'));
		$order->order_status               = '';
		$order->seller_shipping_rules      = $cart->getSellerShipping(array('ruleTitle', 'sellerName', 'total'));

		if ($shippingInProduct)
		{
			$sellerShipping = $cart->getSellerShipping(array('ruleId', 'total'));
			$ruleIds        = array();

			if (!empty($sellerShipping))
			{
				foreach ($sellerShipping as $sellerUid => $rules)
				{
					foreach ($rules as $rule)
					{
						$ruleIds[$sellerUid]['rule_ids'][] = $rule->ruleId;
						$ruleIds[$sellerUid]['total']      = isset($ruleIds[$sellerUid]['total']) ? $ruleIds[$sellerUid]['total'] + $rule->total : $rule->total;
					}
				}
			}

			$order->seller_shipping_rule_ids          = $ruleIds;
			$order->params['product_select_shipping'] = 1;
		}
		else
		{
			$order->seller_shipping_rule_ids = $cart->getSellerShipping('ruleId');
		}
		...
		CheckoutQuestionsFormHelper::saveFormInOrder('order', $cart, $order->id);
		...
		if ($seller_recursive)
		{
			$itemTypes = array_unique(ArrayHelper::getColumn($products, 'product_type'));
			$context   = count($itemTypes) == 1 ? 'order.' . $itemTypes[0] : 'order';

			$this->helper->order->setStatusForSellers($context, 'order_placed', $order->id, true, JText::_('COM_SELLACIOUS_STATUS_TYPE_ORDER_PLACED'));
		}
		else
		{
			$this->helper->order->setStatusByType('order', 'order_placed', $order->id, '', true, true, JText::_('COM_SELLACIOUS_STATUS_TYPE_ORDER_PLACED'));
		}
	 ...
	 * @param   string  $uid      Product code (if the checkoutform is itemised)
	 *
	 * @return  JForm
	 *
	 * @throws  Exception
	 ...
	 * @since   1.4.4
	 */
	public function getCheckoutForm($loadData = false, $context = 'checkout_questions', $uid = null)
	{
		$xml  = '<form> </form>';
		$name = 'com_sellacious.cart.checkoutform';
		$name = $context ? $name . '.' . $context : $name;
		$name = $uid ? $name . '.' . $uid : $name;
		$form = JForm::getInstance($name, $xml, array('control' => 'jform'));
		...
		CheckoutQuestionsFormHelper::prepareCheckOutForm($context, $form, $uid);
		...
		CheckoutQuestionsFormHelper::prepareCheckoutFormData($context, $form, $data, $cart, $uid);
	...
	public function buildCheckoutformData($data)
	{
		...
		$formData = ArrayHelper::getValue($data, 'checkoutform', array(), 'array');
	...
	public function getShippingForms()
	{
		$cart           = $this->getCart();
		$itemisedShip   = $this->helper->config->get('itemised_shipping', SellaciousHelperShipping::SHIPPING_SELECTION_PRODUCT);
		$selectShipping = $this->helper->config->get('product_select_shipping', true);
		$forms          = array();
		...
		elseif ($itemisedShip == SellaciousHelperShipping::SHIPPING_SELECTION_SELLER)
		{
			...
			$sellerQuotes = (array) $cart->getSellerShipQuotes() ?: array();

			foreach ($sellers as $sellerUid => $seller)
			{
				if (isset($sellerQuotes['seller___' . $sellerUid]))
				{
					$quotes = (array) $sellerQuotes['seller___' . $sellerUid];

					if ($selectShipping)
					{
						foreach ($quotes as $itemUid => $quoteItems)
						{
							$cQid = $cart->getSellerShipQuoteId($sellerUid, $itemUid);

							foreach ($quoteItems as $quote)
							{
								if (!is_object($quote))
								{
									// Don't want to entertain flat shipping quotes
									continue;
								}

								if (isset($forms[$sellerUid][$itemUid][$quote->id]))
								{
									continue;
								}

								$form = $this->helper->shippingRule->getForm($quote->ruleId, 'cart[' . $sellerUid . '][' . $itemUid . '][' . $quote->id . ']', $quote->service);
								$xml  = $form ? $form->getXml() : null;
								...
								if ($xml instanceof SimpleXMLElement && count($xml->xpath('//field')))
								{
									if ($cQid == $quote->id)
									{
										$shippingform = $cart->getParam('shippingform.' . $sellerUid . '.' . $itemUid);

										$form->bind($shippingform);
									}

									$forms[$sellerUid][$itemUid][$quote->id] = $form;
								}
							}
						}
					}
					else
					{
						...
						foreach ($quotes as $quote)
						{
							if (isset($forms[$sellerUid][$quote->id]))
							{
								continue;
							}

							$form = $this->helper->shippingRule->getForm($quote->ruleId, 'cart[' . $sellerUid . '][' . $quote->id . ']', $quote->service);
							$xml  = $form ? $form->getXml() : null;
							...
							if ($xml instanceof SimpleXMLElement && count($xml->xpath('//field')))
							{
								if ($cQid == $quote->id)
								{
									$shippingform = $cart->getParam('shippingform.' . $sellerUid);

									$form->bind($shippingform);
								}

								$forms[$sellerUid][$quote->id] = $form;
							}
						}
					}
				}
		...
		elseif ($itemisedShip == SellaciousHelperShipping::SHIPPING_SELECTION_CART)
		{
	...
	protected function saveOrderShipRates($orderId, $cart)
	{
		...
		$selectShipping = $this->helper->config->get('product_select_shipping', 0);

		// If itemised: credit per item directly to whoever concerned
		if ($itemised == SellaciousHelperShipping::SHIPPING_SELECTION_PRODUCT)
		{
			$items = $cart->getItems();

			foreach ($items as $index => $sellerShip)
			{
				// Shall we process Internal items only?
				$iShip = $sellerShip->getShipping();

				if ($iShip->total >= 0.01)
				{
					$sellerUid = $sellerShip->getProperty('seller_uid');

					// Flat shipping
					if ($sellerShip->getShipQuoteId() == null)
					{
						$sellerUid = $shippedBy == 'seller' ? $sellerUid : 0;
					}
					else
					{
						$sellerUid = $iShip->creditAmountTo == 'rule_owner' ? $iShip->ruleOwner : ($iShip->creditAmountTo == 'shop' ? 0 : $sellerUid);
					}

					$records[] = (object) array(
						'order_id'     => $orderId,
						'seller_uid'   => $sellerUid,
						'item_uid'     => $sellerShip->getProperty('code'),
		...
		elseif ($itemised == SellaciousHelperShipping::SHIPPING_SELECTION_SELLER)
		{
			$sShip = $cart->getSellerShipping();

			if ($selectShipping)
			{
				foreach ($sShip as $sellerUid => $itemQuotes)
				{
					/** @var ShippingQuote $sellerShip */
					foreach ($itemQuotes as $uid => $sellerShip)
					{
						if ($sellerShip->total >= 0.01)
						{
							$creditTo = $sellerShip->sellerUid;

							// Flat shipping
							if (!$sellerShip->ruleId)
							{
								$creditTo = $shippedBy == 'seller' ? $creditTo : 0;
							}
							else
							{
								$creditTo = $sellerShip->creditAmountTo == 'rule_owner' ? $sellerShip->ruleOwner : ($sellerShip->creditAmountTo == 'shop' ? 0 : $creditTo);
							}
							...
							$records[]  = (object)array(
								...
								'seller_uid'   => $creditTo,
								'item_uid'     => $uid,
								'rule_id'      => $sellerShip->ruleId,
								'rule_title'   => $sellerShip->ruleTitle,
								'rule_handler' => $sellerShip->ruleHandler,
								'amount'       => $sellerShip->total,
								'tbd'          => $sellerShip->tbd,
							);
						}
					}
			...
			{
				foreach ($sShip as $sQuote => $sellerShip)
				{
					if ($sellerShip->total >= 0.01)
					{
						$creditTo = $sellerShip->sellerUid;
		...
		// Cart-wise shipping, Flat shipping (null): Use Collated Groups
		elseif ($cart->getShipQuoteId() === null)
		{
			// shipped by Shop: Credit the shop directly
			if ($shippedBy == 'shop')
			{
			...
			else
			{
				...
				foreach ($quoteList as $sUid => $quotes)
				{
					foreach ($quotes as $pid => $quote)
					{
						...
						{
							$sUid      = $quote['creditAmountTo'] == 'rule_owner' ? $quote['ruleOwner'] : ($quote['creditAmountTo'] == 'shop' ? 0 : $sUid);
							$records[] = (object)array(
								'order_id'     => $orderId,
								'seller_uid'   => $sUid,
								'item_uid'     => $pid,
		...
		// Cart-wise shipping, Rule based shipping: Use Collated Groups
		else
		{
