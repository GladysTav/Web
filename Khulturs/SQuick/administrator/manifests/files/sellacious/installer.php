...
class SellaciousInstallerScript
{
	 ...
	 * @since   1.0.0
	 */
	public function getExtensionInstall(array &$queries)
	{
		// Nowdoc tag XML_FORMAT is important for this file to be processed by build script
		$manifest = <<<'XML_FORMAT'
<extension>
...
XML_FORMAT;
		...
		foreach ($manifest as $ek => $ev)
		{
			$mCache[$ek] = (string) $ev;
		}

		$mCache     = json_encode($mCache);
		$columns    = 'name, type, element, folder, client_id, enabled, access, protected, manifest_cache';
		$extensions = array(
			array('sellacious', 'template', 'sellacious', '', 2, 1, 1, 1, $mCache),
			array('mod_custom', 'module', 'mod_custom', '', 2, 1, 1, 1, $mCache),
			array('mod_login', 'module', 'mod_login', '', 2, 1, 1, 1, $mCache),
			array('mod_toolbar', 'module', 'mod_toolbar', '', 2, 1, 1, 1, $mCache),
			array('mod_smartymenu', 'module', 'mod_smartymenu', '', 2, 1, 2, 1, $mCache),
			array('mod_title', 'module', 'mod_title', '', 2, 1, 1, 1, $mCache),
			array('mod_breadcrumbs', 'module', 'mod_breadcrumbs', '', 2, 1, 1, 1, $mCache),
			array('mod_footer', 'module', 'mod_footer', '', 2, 1, 1, 1, $mCache),
		);
	...
	protected function getModuleInstall(array &$queries)
	{
		...
		$modules = array(
			array('Title', '', 1, 'title', 1, 'mod_title', 1, 1, '', 2, '*'),
	...
	/**
	 * Since `v2.0.0` we use `pricing_types`. The `pricing_model` and `price_display` are deprecated
	 *
	 * @return  void
	 *
	 * @since   2.0.0
	 */
	protected function fixPriceDisplay()
	{
		try
		{
			...
			$pTypes = $helper->config->get('pricing_types');

			// If pricing_types is already set, nothing to do here.
			if  ($pTypes)
			{
				return;
			}
			...
			$types   = array();
			$model   = $helper->config->get('pricing_model');
			$allowed = (array) $helper->config->get('allowed_price_display') ?: array(0, 1, 2, 3);
			...
			$helper->config->set('pricing_model', $types);

			$db     = JFactory::getDbo();
			$query  = $db->getQuery(true);
			$values = array($model == 'basic' || $model == 'flat' ? $model : 'dynamic', 'call', 'email', 'queryform');

			foreach ($values as $pd => $pt)
			{
				$query->clear();
				$query->update('#__sellacious_product_sellers')
				      ->set('pricing_type = ' . $db->q($pt))
				      ->where('price_display = ' . $db->q($pd));

				// Only update what is not already set
				$query->where('pricing_type = ' . $db->q(''));
		...
		catch (Exception $e)
		{
			try
			{
				...
				$app->enqueueMessage('Could not update pricing types settings. Please do it manually from the settings page in sellacious.', 'warning');
			}
			catch (Exception $e)
			{
			}
