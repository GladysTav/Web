						...
						<div class="product-item-details">
							<div class="product-item-inner">
								<div class="product-item-name-block">
									...
									$dispatcher->trigger('onRenderCartPrices', array('com_sellacious.cart', $item, &$additionalPrices));

									foreach ($additionalPrices as $additionalPrice)
									{
										?>
										<div><?php echo $additionalPrice; ?></div>
										<?php
									}
									?>
								</div>
...
<table class="cart-items-table">
	...
	<?php if (abs($totals->get('shipping')) >= 0.01): ?>
		...
		<?php if ($itemisedShip && abs($totals->get('shipping_tax_amount')) >= 0.01): ?>
			<tr>
				<td class="strong">
					<?php echo JText::_('COM_SELLACIOUSOPC_CART_LABEL_TOTAL_TAX_ON_SHIPPING') ?>
				</td>
				<td class="strong text-right">
					<span><?php echo $helper->currency->display($totals->get('shipping_tax_amount'), $g_currency, '', true); ?></span>
				</td>
			</tr>
		<?php endif; ?>

		<?php if ($itemisedShip && abs($totals->get('shipping_discount_amount')) >= 0.01): ?>
			<tr>
				<td class="strong">
					<?php echo JText::_('COM_SELLACIOUSOPC_CART_LABEL_TOTAL_TAX_ON_DISCOUNT') ?>
				</td>
				<td class="strong text-right">
					<span><?php echo $helper->currency->display($totals->get('shipping_discount_amount'), $g_currency, '', true); ?></span>
				</td>
			</tr>
		<?php endif; ?>
				...
				if (($rule->type == 'tax' || $rule->type == 'discount') && $rule->sum_method == 3)
				{
					...
					<tr>
						<td>
							...
							<?php echo $this->escape($rule->title) ?> @ <?php echo $rule->percent ? sprintf('%s%%', number_format($rule->amount, 2)) :
								$helper->currency->display($rule->amount, $g_currency, '', true); ?>
							<label class="label label-info"><?php echo JText::_('COM_SELLACIOUS_CART_LABEL_' . strtoupper($rule->type) .'_ON_SHIPPING') ?></label>
						</td>
						<td class="text-right"><?php echo $helper->currency->display(abs($rule->change), $g_currency, '', true); ?></td>
					</tr>
					<?php
	...
	<?php if ($round_enabled): ?>
		<tr>
			<td class="strong">
				<?php echo JText::_('COM_SELLACIOUSOPC_CART_ROUNDED_GRAND_TOTAL_LABEL') ?>
				<?php echo $totals->get('ship_tbd') ? '<span class="red"> *</span>' : '' ?>:
			</td>
			<td class="strong text-right">
				<span class="grand-total strong" data-amount="<?php echo $totals->get('grand_total_rounded') ?>">
				<?php echo $helper->currency->display($totals->get('grand_total_rounded'), $g_currency, '', true); ?>
			</span>
			</td>
		</tr>
	<?php endif; ?>
