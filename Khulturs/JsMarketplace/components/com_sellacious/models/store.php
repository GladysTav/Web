...
class SellaciousModelStore extends SellaciousModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$this->state->set('filter.client_catid', $catId);
	}
	...
	protected function getListQuery()
	{
		...
		$fFields  = $this->helper->field->loadColumn(array('list.select' => 'a.id', 'filterable' => 1, 'state' => 1, 'list.where' => 'a.type <> \'fieldgroup\''));
		$keywords = $this->getState('filter.query');

		foreach (array_filter(explode(' ', $keywords)) as $keyword)
		{
			$cond = array();
	...
	protected function getItemsQuery()
	{
		...
		$query->select($this->_db->quoteName(array_values($columns), array_keys($columns)))
			...
			->where('CASE WHEN (a.variant_id > 0) THEN CASE WHEN (a.variant_active = 1) THEN 1 ELSE 0 END ELSE 1 END');
		...
		$catId = $this->state->get('filter.client_catid');
	...
	public function getItems()
	{
		...
		try
		{
			...
			// $list = $this->_getList($this->_getListQuery(), $this->getStart(), $this->getState('list.limit'));

			// $this->cache[$store] = $list;

			/** @var   CallbackController  $cache */
			$cache = \JCacheControllerView::getInstance('callback');
			$cache->cache->setCaching(true);
			$list  = $cache->get(array($this, 'getListWrapper'), array($this->_getListQuery(), $this->getStart(), $this->getState('list.limit')));
	...
	public function getListWrapper($q, $o, $l)
	{
		return $this->_getList($q, $o, $l);
	}
	...
	protected function processList($items)
	{
		...
		foreach ($items as &$item)
		{
			...
			$item->rendered_attributes = array();
		}

		$dispatcher->trigger('onProcessProducts', array('com_sellacious.store', $items));
