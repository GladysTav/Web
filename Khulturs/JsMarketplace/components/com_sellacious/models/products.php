...
class SellaciousModelProducts extends SellaciousModelList
{
	...
	protected function processList($items)
	{
		...
		foreach ($items as &$item)
		{
			...
			$prices = array();
			$dispatcher->trigger('onLoadPrices', array('com_sellacious.product', $item, &$prices));
			$item->pricing_plans = $prices;

			// Initializing attributes
			$item->rendered_attributes = array();
		}

		$dispatcher->trigger('onProcessProducts', array('com_sellacious.products', $items));
