...
$listOptions = array(
	JHtml::_('select.option', 1, JText::_('COM_SELLACIOUS_FROM')),
	JHtml::_('select.option', 2, JText::_('COM_SELLACIOUS_NOT_FROM'))
);

$listId   = $field->id . '_type';
$listName = $field->name . '[type]';
$listValue = 1;

if (!empty($field->value) && isset($field->value[0]['type']))
{
	$listValue = $field->value[0]['type'];
}
?>
<fieldset class="w100p" id="<?php echo $field->id ?>" <?php echo $class . $field->required . $field->autofocus . $field->disabled ?>>
	<?php echo JHtml::_('select.genericlist', $listOptions, $listName, 'class="tiny-input margin-right-5"', 'value', 'text', $listValue, $listId); ?>
	<?php
