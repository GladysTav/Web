		...
		if (count($shipping_shoprules))
		{
			foreach ($shipping_shoprules as $ri => $rule)
			{
				if (abs($rule->change) >= 0.01)
				{
					// $rule = {level, title, percent, amount, input, change, output};
					?>
					<tr class="shoprule-info <?php echo $item->getUid() ?>-info-shipping hidden">
						<td colspan="<?php echo $itemisedShip ? '8' : '7' ?>">
							<?php echo str_repeat('|&mdash;', $rule->level - 1) ?>
							<?php echo $this->escape($rule->title) ?> @ <?php echo $rule->percent ? sprintf('%s%%', number_format($rule->amount, 2)) :
								$helper->currency->display($rule->amount, $g_currency, '', true); ?>
			...
			?>
			<tr class="<?php echo $item->getUid() ?>-info-shipping hidden">
				<th colspan="8"> </th>
			</tr>
			<?php
		}
	...
	<tr>
		...
		<td colspan="4" class="v-top noborder">
			...
			<?php if ($round_enabled): ?>
				<div class="total-amount"><?php echo JText::_('COM_SELLACIOUS_CART_ROUNDED_GRAND_TOTAL_LABEL') ?>
					<?php echo $totals->get('ship_tbd') ? '<span class="red"> *</span>' : '' ?>:
					<span class="grand-total strong nowrap" data-amount="<?php echo $totals->get('grand_total_rounded') ?>">
