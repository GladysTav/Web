...
$showCart        = $this->helper->config->get('cart_modal');
...
{
	$reviewsUrl = JRoute::_('index.php?option=com_sellacious&view=reviews&product_id=' . $item->id . '&variant_id=' . (int) $variantId);
}
else
{
...
<div class="product-wrap <?php echo $sellCols ?>">
	<div class="product-box spl-cat-<?php echo (int) $item->spl_listing_catid; ?>" data-rollover="container" data-id="<?php echo $item->id; ?>" data-seller="<?php echo $item->seller_uid; ?>" data-product-code="<?php echo $item->code; ?>" data-url="<?php echo $url; ?>">
		...
		<div class="product-info-box">
						...
						<?php if ((int) $item->stock_capacity > 0): ?>
							<?php if ($allow_checkout): ?>
								<?php if (in_array('products', $cart_pages)): ?>
									...
									?>
									<?php if (count($item->pricing_plans) > 1 && $link_detail): ?>
										<a href="<?php echo $url ?>" class="btn btn-primary btn-product-detail">
								...
								<?php if (in_array('products', $buynow_pages)): ?>
									<?php if (count($item->pricing_plans) > 1 && $link_detail): ?>
										<a href="<?php echo $url ?>" class="btn btn-default btn-product-detail buy">
											<i class="fa fa-bolt" aria-hidden="true"></i> <?php echo JText::_('COM_SELLACIOUS_PRODUCT_BUY_NOW'); ?>
										</a>
									<?php else: ?>
										<button type="button" class="btn btn-default btn-add-cart buy" data-item="<?php echo $item->code; ?>"
												data-checkout="true">
											<i class="fa fa-bolt" aria-hidden="true"></i> <?php echo JText::_('COM_SELLACIOUS_PRODUCT_BUY_NOW'); ?>
										</button>
									<?php endif; ?>
							...
							<?php if (in_array('products', $show_modal)): ?>
								<a href="#sell-modal-<?php echo $item->code; ?>" role="button" data-toggle="modal"
								   class="btn btn-default btn-quick-view">
			...
			<?php if ($this->helper->access->check('product.edit') ||
			          ($this->helper->access->checkAny(array(
					          'basic.own',
					          'seller.own',
					          'pricing.own',
					          'shipping.own',
					          'related.own',
					          'seo.own',
				          ), 'product.edit.', $item->id) && $item->seller_uid == $me->id)): ?>
