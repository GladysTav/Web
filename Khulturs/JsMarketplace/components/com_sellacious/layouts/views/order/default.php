	...
	<?php if (!empty($items)): ?>
			...
			foreach ($items as $oi)
			{
				...
				<tr>
					...
					<td class="v-top">
						...
						$dispatcher->trigger('onRenderOrderItem', array('com_sellacious.order', $oi, &$additionalInfo));
					...
					<td class="text-right nowrap v-top item-total">
						...
						<?php endif; ?>
						<?php if (abs($oi->shipping_taxes) >= 0.01): ?>
						<small><?php echo JText::_('COM_SELLACIOUS_ORDER_ITEM_SHIPPING_TAX_AMOUNT_LABEL') ?>
						<?php echo $this->helper->currency->display($oi->shipping_taxes, $o_currency, $c_currency, true); ?></small><br />
						<?php endif; ?>
						<?php if (abs($oi->shipping_discounts) >= 0.01): ?>
						<small><?php echo JText::_('COM_SELLACIOUS_ORDER_ITEM_SHIPPING_DISCOUNT_AMOUNT_LABEL') ?>
						<?php echo $this->helper->currency->display($oi->shipping_discounts, $o_currency, $c_currency, true); ?></small><br />
						<?php endif; ?>
						<?php
		...
		<table class="w100p shoprule-info">
			...
			<?php if (abs($order->get('product_shipping')) >= 0.01): ?>
				...
				<?php
				$shipping_shoprules = (array) $order->get('shipping_shoprules');
