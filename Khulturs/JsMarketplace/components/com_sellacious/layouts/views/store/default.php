...
$seller->reviewsUrl = JRoute::_('index.php?option=com_sellacious&view=reviews&seller_uid=' . $seller->get('user_id'));
$seller->reviews = $this->helper->seller->getSellerReviews($seller->get('user_id'));
?>
<div id="seller-info">
	...
	<div class="sellerdata">
		...
		<?php endif; ?>
		<?php if ($this->helper->config->get('store_details_show_rating')): ?>
			<?php if (in_array('seller', $rateable)): ?>
				...
				<div class="product-rating rating-stars star-<?php echo $stars; ?>">
					<a href="<?php echo $seller->reviewsUrl ?>">
...
<?php
if($this->helper->config->get('show_store_reviews') == '1' && !empty($seller->reviews)) {
	echo $this->loadTemplate('reviews', $seller);
}
?>
