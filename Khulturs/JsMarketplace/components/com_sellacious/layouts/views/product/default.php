...
$manufacturer_disp  = (array) $this->helper->config->get('manufacturer_details_display');
...
<div class="product-single">
	<div class="sell-row">
		...
		<div id="product-info" class="sell-col-md-7">
			...
			<?php if (isset($manufacturer->id) && in_array('product', $manufacturer_disp)): ?>
			...
			<div class="sell-row">
				...
				<div class="<?php echo ($marketPlace || $conditionbox || $exchangeReturn) ? 'sell-col-xs-7' : 'sell-col-xs-12' ?>">
					...
					<div class="qtyarea">
						<?php
						$showCartModal = $this->helper->config->get('cart_modal');
				...
				<?php if ($marketPlace || $conditionbox || $exchangeReturn): ?>
					<div class="sell-col-xs-5">
						<div class="product-actions">
							...
							<?php if (($showlisting) && $item->get('type') != 'electronic'): ?>
			...
			<?php if ((in_array('product', (array) $this->helper->config->get('product_features_list'))) || (count($offers = $item->get('offers'))) || (count($taxes = $item->get('taxes')))): ?>
				<hr class="isolate"/>
				<div class="sell-row">
					...
					<?php if (count($taxes = $item->get('taxes'))): ?>
						<div class="sell-col-xs-12">
							<hr class="isolate"/>
							<div class="tax-info">
								<h4 class="tax-info-header"><?php echo JText::plural('COM_SELLACIOUS_PRODUCT_TAX_COUNT_N', count($taxes)) ?> | <?php
									echo JText::_('COM_SELLACIOUS_PRODUCT_APPLICATION_ON_CHECKOUT'); ?></h4>
								<div class="taxeslist">
									<?php
	...
	if ($this->helper->config->get('product_rating')): ?>
		<div class="rating-box sell-infobox">
			<h3><?php echo JText::_('COM_SELLACIOUS_TITLE_RATINGS') ?></h3>
			<?php
