...
class SellaciousHelperProduct extends SellaciousHelperBase
{
	...
	public function setSellerAttributesByType($attribs, $product_id, $seller_uid, $type)
	{
		...
		$psx = array(
			...
			'product_address'  => ArrayHelper::getValue($attribs, 'product_address'),
			'product_location' => ArrayHelper::getValue($attribs, 'product_location'),
			'state'            => ArrayHelper::getValue($attribs, 'state'),
		);

		// Save Seller Product Geolocation
		$geolocation              = (object) ArrayHelper::getValue($attribs, 'geolocation');
		$geolocation->address     = ArrayHelper::getValue($attribs, 'product_address');
		$geolocation->coordinates = ArrayHelper::getValue($attribs, 'product_location');

		$this->helper->location->saveGeoLocation('product_sellers', $table->get('id'), (array) $geolocation);
		...
		unset($attribs['price_display'], $attribs['query_form'], $attribs['over_stock'], $attribs['stock'], $attribs['state'], $attribs['rules']);
	...
	public function getModProducts($params, $type = 'latest')
	{
		...
		if (!empty($categories))
		{
			...
			{
				$allCategories = $this->helper->category->getChildren($category, true);
			}
		...
		$query->where('CASE WHEN (a.variant_id > 0) THEN CASE WHEN (a.variant_active = 1) THEN 1 ELSE 0 END ELSE 1 END');
		...
		$dispatcher->trigger('onProcessProducts', array('com_sellacious.products.' . $type, $products, $params->toArray()));
