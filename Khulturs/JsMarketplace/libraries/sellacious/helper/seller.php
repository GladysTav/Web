...
class SellaciousHelperSeller extends SellaciousHelperBase
{
	...
	public function getShipLocations($seller_uid, $grouped = false)
	{
		...
		if ($grouped)
		{
			$query->select('g.type')->join('left', '#__sellacious_locations AS g ON (CASE WHEN g.type = \'zip\' THEN g.title = a.gl_id ELSE g.id = a.gl_id END)');
	 ...
	 * @since    1.7.0
	 */
	public function getSellerReviews($seller_uid)
	{
		$db     = JFactory::getDbo();
		$result = array();
		$query = $db->getQuery(true);
