...
class SellaciousView extends JViewLegacy
{
	...
	public function __construct($config = array())
	{
		...
		//If the view has an active menu and template style selected, set it as the default template for the layout
		$menu = $app->getMenu();
		...
		if ($active)
		{
			...
			$query->select('template, params')->from('#__template_styles')->where('id = ' . $active->template_style_id);
			$db->setQuery($query);

			$templateStyle = $db->loadObject();

			if ($templateStyle)
			{
				$app->setTemplate($templateStyle->template, $templateStyle->params);
			}
		}
