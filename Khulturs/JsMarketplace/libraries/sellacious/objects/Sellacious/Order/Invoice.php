...
class Invoice extends BaseObject
{
	 ...
	 * @since   1.7.0
	 */
	public function __construct($order_id, $seller_uid)
	{
	 ...
	 * @since   1.7.0
	 */
	public function createSellerInvoice()
	{
		...
		$oItems = $this->helper->order->getOrderItems($this->order_id, null, array('a.seller_uid = ' . $this->seller_uid));
	...
	public function getInvoiceData()
	{
		...
		$invoiceData = $invoice->get('invoice_data');

		if (empty($invoiceData))
		{
			$this->createSellerInvoice();
			$invoice->load(array('order_id' => $this->order_id, 'seller_uid' => $this->seller_uid));

			$invoiceData = $invoice->get('invoice_data');
		}

		$this->invoice_data = new Registry($invoiceData);
