...
class Products extends Cache
{
	 ...
	 * @since  1.7.0
	 */
	protected $cacheTable = '#__sellacious_cache_products';
	 ...
	 * @since  1.7.0
	 */
	protected $tmpCacheTable = '#__sellacious_cache_temp_products';
	...
	public function build()
	{
		...
		$query->update($this->db->quoteName($this->tmpCacheTable))->set('state = 1')->where('state = 0');
		$this->db->setQuery($query)->execute();

		// Copy temporary table to real table
		$this->writeCacheTable();
	}
	...
	{
		// We'd directly refresh the main table
		$tmp = $this->tmpCacheTable;
		$this->tmpCacheTable = $this->cacheTable;
		...
		if ($key == 'products')
		{
			...
			$query = $this->db->getQuery(true);
			$query->delete($this->db->quoteName($this->tmpCacheTable))
		...
		elseif ($key == 'users')
		{
			...
			$query->update($this->db->quoteName($this->tmpCacheTable))
				->set('state = 0')
		...
		elseif ($key == 'prices')
		{
			...
			foreach ($iterator as $obj)
			{
				...
				$this->db->updateObject($this->tmpCacheTable, $obj, array('product_id', 'seller_uid'));
			}
		...
		// Reset property
		$this->tmpCacheTable = $tmp;

		// Reset expired - not expired
		$query = $this->db->getQuery(true);
		$query->update($this->db->quoteName($this->cacheTable))->set('state = 1')->where('state = 0');
		$this->db->setQuery($query)->execute();
	}
	...
	protected function productsCache($query, $columns)
	{
		...
		$insert = sprintf(
			...
			$this->db->quoteName($this->tmpCacheTable),
	...
	protected function updateManufacturer($state = false)
	{
		...
		$query->update($this->db->quoteName($this->tmpCacheTable, 'a'))
	...
	protected function updateForex($state = false)
	{
		...
		$query->select('DISTINCT a.seller_currency')
			->from($this->db->quoteName($this->tmpCacheTable, 'a'));
		...
		foreach ($iterator as $cObj)
		{
			...
			$this->db->updateObject($this->tmpCacheTable, $cObj, array('seller_currency'));
		}
	}
	...
	protected function updateSpecFields()
	{
		...
		foreach ($iterator as $item)
		{
			...
			$this->db->updateObject($this->tmpCacheTable, $item, array('category_ids'));
		}
	}
	...
	protected function updateSalesPrice($state = false, $values = null)
	{
		...
		$query->from($this->db->quoteName($this->tmpCacheTable, 'a'));
		$query->join('LEFT', $this->db->quoteName('#__sellacious_product_prices', 'p') . ' ON p.product_id = a.product_id AND p.seller_uid = a.seller_uid AND p.is_fallback = 1');
	...
	protected function updateProductRating($state = false, $values = null)
	{
		...
		$subQ->select(array('r.product_id', 'r.variant_id', 'COUNT(r.rating) AS rating_count', 'SUM(r.rating) AS rating_total'))
			->from($this->db->qn('#__sellacious_ratings', 'r'))
			->where('r.type = ' . $this->db->q('product'))
			->where('r.state = 1')
			->where('r.rating > 0')
			->group('r.product_id' . ($variantSeparate ? ', r.variant_id' : ''));

		$query->update($this->db->quoteName($this->tmpCacheTable, 'a'))
			->set('a.product_rating = rc.rating_total / rc.rating_count')
			->join('inner', '(' . $subQ . ') AS rc' . ' ON rc.product_id = a.product_id AND rc.rating_count > 0' . ($variantSeparate ? ' AND rc.variant_id = a.variant_id' : ''));
	...
	protected function updateProductCode($state = false)
	{
		...
		foreach ($iterator as $cObj)
		{
			...
			$this->db->updateObject($this->tmpCacheTable, $cObj, array('product_id', 'variant_id', 'seller_uid'));
		}
	}
