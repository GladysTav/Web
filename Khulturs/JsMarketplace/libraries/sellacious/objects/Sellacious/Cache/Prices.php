...
class Prices extends Cache
{
	 ...
	 * @since  1.7.0
	 */
	protected $cacheTable = '#__sellacious_cache_prices';
	 ...
	 * @since  1.7.0
	 */
	protected $tmpCacheTable = '#__sellacious_cache_temp_prices';
	...
	public function build()
	{
		...
		// Purge existing
		$this->db->truncateTable($this->tmpCacheTable);
	...
	public function refresh($key, $values)
	{
		...
		if ($key == 'products')
		{
			...
			$query->delete($this->db->quoteName($this->tmpCacheTable))->where('product_id IN (' . implode(', ', $pks) . ')');
		...
		elseif ($key == 'prices')
		{
			...
			$pks   = array_map('intval', (array) $values);
			$query = $this->db->getQuery(true);
			$query->delete($this->db->quoteName($this->tmpCacheTable))->where('price_id IN (' . implode(', ', $pks) . ')');
			$this->db->setQuery($query)->execute();
