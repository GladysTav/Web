		...
		<script>
			jQuery(document).ready(function ($) {
				if ($('#modal-cart').length == 0) {
					...
					$cartModal.find('.modal-body').html('<div id='
					cart - items
					'></div>'
				)
					;
			...
			<div class="sellerproducts-product-box <?php echo $splCategoryClass; ?>" data-rollover="container">
				<div class="image-box">
					<a href="<?php echo $url; ?>">
						<img src="<?php echo reset($images); ?>" data-rollover='<?php echo htmlspecialchars(json_encode($images)); ?>'
							 title="<?php echo $item->product_title; ?>">
					</a>
				...
				<div class="sellerproducts-product-info-box">
					...
					elseif ($seller_attr->price_display == 0)
					{
						...
						{
							$price = round($item->sales_price, 2) >= 0.01 ? $helper->currency->display($item->sales_price, $s_currency, $c_currency, true) : JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_PRICE_FREE');
								...
								<?php if ((int) $seller_attr->stock_capacity > 0): ?>
									...
									<?php if ($displaybuynowbtn == '1'): ?>
										<button type="button" class="btn btn-default btn-add-cart buy" data-item="<?php echo $code; ?>"
												data-checkout="true">
											<i class="fa fa-bolt"
											   aria-hidden="true"></i> <?php echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_BUY_NOW'); ?>
										</button>
									<?php endif; ?>
								<?php else: ?>
									<hr class="isolate">
