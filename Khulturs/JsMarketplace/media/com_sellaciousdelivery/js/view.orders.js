...
(function ($) {
	...
	$(document).ready(function () {
		...
		var rebuildItemRows = function (status, row) {
			...
			var new_delivery_status = status['delivery_status'];

			var idx = row.attr('id').match(/\d+-\d+/);
			var oi_row = $('#oi-row-' + idx);

			oi_row.find('.oi-status').text(new_status);
			oi_row.find('.delivery-status').html('<td class="' + new_delivery_status.class + '">' + new_delivery_status.status + '</td>');
		...
		$('.btn-oi-status-edit').click(function (e) {
			...
			var node = '#oi-row-' + row_index[1] + '-' + row_index[2];
		...
		$('#list_delivery_due input[type="radio"]').on('change', function () {
			syncDeliveryFilter();
		...
		var syncDeliveryFilter = function () {
			var deliveryDue = $('#list_delivery_due input[type="radio"]:checked').val();

			if (deliveryDue == 'select_date') {
				$('#list_delivery_due_date').closest('.js-stools-field-list').removeClass('hidden');
			} else {
				$('#list_delivery_due_date').closest('.js-stools-field-list').addClass('hidden');
			}
		};
