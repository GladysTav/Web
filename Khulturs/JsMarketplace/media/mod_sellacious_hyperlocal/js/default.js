...
(function ($) {
	ModSellaciousHyperLocal.prototype = {
		...
		setup: function (options) {
			...
			if ($this.options.params.address_autocomplete == 2)
			{
				...
				$this.autocomplete.addListener('place_changed', function () {
					...
					if ($this.options.params.address_matching == 1) {
						$this.setAddress('', formatted_address, $this.setBounds);
					} else if($this.options.params.address_matching == 2) {
						place.formatted_address = formatted_address;
						$this.setGeoLocation(place, 1, function () {
		...
		geolocate: function(location) {
			...
			if ($this.options.params.browser_detect == 1 && (location === undefined || !Object.keys(location).length))
			{
				if (navigator.geolocation) {
                    ...
                    $($geo_finder_btn).html('<i class="fa fa-spin fa-spinner"></i>');
						...
						$this.geocoder.geocode({
							...
							if (status === google.maps.GeocoderStatus.OK) {
								if (results[1]) {
									if ($this.options.params.address_autocomplete == 2) {
										if ($this.options.params.address_matching == 1) {
											$this.setGeoLocation(results[1], 0, $this.setBounds);
										} else if($this.options.params.address_matching == 2) {
											$this.setGeoLocation(results[1], 1, function () {
												window.location.reload()
											});
										}
									} else {
		...
		setCustomAddress: function(address, lat, lng, callback) {
			var $this = this;
		...
		setGeoLocation: function(result, setTimeZone, callback) {
			var $this = this;
			...
			if (setTimeZone == 1) {
				...
				$this.geojax = $.ajax({
					url:"https://maps.googleapis.com/maps/api/timezone/json?location="+lat+","+long+"&timestamp="+(Math.round((new Date().getTime())/1000)).toString()+'&key='+$this.options.params.google_api_key,
					async: false
				}).done(function(response){
					data.timezone = response.timeZoneId;
				});
			}
