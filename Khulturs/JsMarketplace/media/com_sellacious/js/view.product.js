...
(function ($) {
	...
	SellaciousViewProduct.Variant.prototype = {
		...
		hideForm: function () {
			...
			var $fieldset = $that.element.find('.add-variant-form').removeClass('in');
		...
		fillForm: function ($fieldset, data) {
			...
			$fieldset.find('#jform_variant_state').find('input[type="radio"]').each(function () {
				var active = ($(this).val() == data['state']);
		...
		showForm: function (data) {
			...
			var $fieldset = $that.element.find('.add-variant-form');
	...
	$(document).ready(function () {
		...
		$(document).on('onMapChangeLocation', function (event, lat, lng, id) {
			if (id == 'jform_seller_product_address') {
				var location = (Math.round(lat * 1000000) / 1000000) + ',' + (Math.round(lng * 1000000) / 1000000);
				$('#jform_seller_product_location').val(location);
			}
		});

		$(document).on('OnMapGeoCode', function (event, address_components, id) {
			if (id == 'jform_seller_product_address') {
				// Empty components first
				$('#jform_seller_geolocation_locality').val('');
				$('#jform_seller_geolocation_sublocality').val('');
				$('#jform_seller_geolocation_city').val('');
				$('#jform_seller_geolocation_district').val('');
				$('#jform_seller_geolocation_state').val('');
				$('#jform_seller_geolocation_country').val('');
				$('#jform_seller_geolocation_zip').val('');

				$.each(address_components, function (key, component) {
					if ($.inArray('sublocality_level_1', component.types) != -1) {
						$('#jform_seller_geolocation_locality').val(component.long_name);
					} else if ($.inArray('sublocality_level_2', component.types) != -1) {
						$('#jform_seller_geolocation_sublocality').val(component.long_name);
					} else if ($.inArray('locality', component.types) != -1) {
						$('#jform_seller_geolocation_city').val(component.long_name);
					} else if ($.inArray('administrative_area_level_2', component.types) != -1) {
						$('#jform_seller_geolocation_district').val(component.long_name);
					} else if ($.inArray('administrative_area_level_1', component.types) != -1) {
						$('#jform_seller_geolocation_state').val(component.long_name);
					} else if ($.inArray('country', component.types) != -1) {
						$('#jform_seller_geolocation_country').val(component.long_name);
					} else if ($.inArray('postal_code', component.types) != -1) {
						$('#jform_seller_geolocation_zip').val(component.long_name);
					}
				});
			}
		});

		$('#jform_seller_product_location').on('change', function () {
