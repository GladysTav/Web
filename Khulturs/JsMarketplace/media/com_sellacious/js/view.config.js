...
(function ($) {
	$(document).ready(function () {
		...
		$(':input').on('change click', function () {
			if ($('#jform_jconfig_sef0').is(':checked')) {
				$('#jform_jconfig_sef_rewrite').closest('.input-row').show();
				$('#jform_jconfig_sef_suffix').closest('.input-row').show();
				$('#jform_jconfig_unicodeslugs').closest('.input-row').show();
			} else {
				$('#jform_jconfig_sef_rewrite').closest('.input-row').hide();
				$('#jform_jconfig_sef_suffix').closest('.input-row').hide();
				$('#jform_jconfig_unicodeslugs').closest('.input-row').hide();
			}

			if ($('#jform_jconfig_display_offline_message').val() == 1) {
				$('#jform_jconfig_offline_message').closest('.input-row').show();
			} else {
				$('#jform_jconfig_offline_message').closest('.input-row').hide();
			}
			...
			// Forex API
			var forexApi = $('#jform_com_sellacious_forex_api input[type="radio"]:checked').val();

			switch (forexApi) {
				default:
					$('#jform_com_sellacious_fixer_access_key').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_layer_access_key').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_converter_api_mode1').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_converter_access_key').closest('.input-row').hide();
					break;

				case 'Fixer':
					$('#jform_com_sellacious_fixer_access_key').closest('.input-row').show();
					$('#jform_com_sellacious_currency_layer_access_key').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_converter_api_mode1').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_converter_access_key').closest('.input-row').hide();
					break;
				...
				case 'CurrencyLayer':
					$('#jform_com_sellacious_fixer_access_key').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_layer_access_key').closest('.input-row').show();
					$('#jform_com_sellacious_currency_converter_api_mode1').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_converter_access_key').closest('.input-row').hide();
					break;
				...
				case 'CurrencyConverter':
					$('#jform_com_sellacious_fixer_access_key').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_layer_access_key').closest('.input-row').hide();
					$('#jform_com_sellacious_currency_converter_api_mode1').closest('.input-row').show();
					$('#jform_com_sellacious_currency_converter_access_key').closest('.input-row').show();
					break;
			}

			if ($allowed_product_type == 'both') {
				$('#jform_com_sellacious_default_product_type').closest('.input-row').show();
			} else {
				$('#jform_com_sellacious_default_product_type').closest('.input-row').hide();
			}
			...
			if ($('#jform_com_sellacious_product_category_limit2').is(':checked')) {
				$('#jform_com_sellacious_product_category_limit_value').closest('.input-row').show();
			} else {
				$('#jform_com_sellacious_product_category_limit_value').closest('.input-row').hide();
			}

			// Seller Limit
			if ($('#jform_com_sellacious_seller_limit input[type="radio"]:checked').val() == 0) {
				$('#jform_com_sellacious_seller_limit_value').closest('.input-row').hide();
			} else {
				$('#jform_com_sellacious_seller_limit_value').closest('.input-row').show();
			}

			// Google Analytics
			if ($('#jform_com_sellacious_google_analytics input[type="radio"]:checked').val() == 1) {
				$('#jform_com_sellacious_ga_code').closest('.input-row').show();
				$('#jform_com_sellacious_ga_script').closest('.input-row').hide();
			} else {
				$('#jform_com_sellacious_ga_code').closest('.input-row').hide();
				$('#jform_com_sellacious_ga_script').closest('.input-row').show();
			}

			// FB Pixel
			if ($('#jform_com_sellacious_facebook_pixel input[type="radio"]:checked').val() == 1) {
				$('#jform_com_sellacious_fb_pixel_code').closest('.input-row').show();
				$('#jform_com_sellacious_fb_pixel_script').closest('.input-row').hide();
			} else {
				$('#jform_com_sellacious_fb_pixel_code').closest('.input-row').hide();
				$('#jform_com_sellacious_fb_pixel_script').closest('.input-row').show();
			}
		...
		$(document).on('onMapChangeLocation', function (event, lat, lng, id) {
			if (id == 'jform_com_sellacious_contact_address') {
				var location = (Math.round(lat * 1000000) / 1000000) + ',' + (Math.round(lng * 1000000) / 1000000);
				$('#jform_com_sellacious_contact_address_location').val(location);
			}
		});
	});
})(jQuery);
