...
class PlgSystemSellaciousMailer extends SellaciousPlugin
{
	...
	protected function sendMail($email)
	{
		...
		if (!empty($params['attachments']))
		{
			...
			{
				$path               = JPATH_SITE . '/' . $attachment['path'];
			...
			if (trim($body) != '' && count($recipients) != 0)
			{
				...
				if ($sent === true)
				{
					...
					if (isset($params['attachments']) && is_array($params['attachments']))
					{
						foreach ($params['attachments'] as $attachment)
						{
							if (isset($attachment['name']) && in_array($attachment['name'], $attachments) && isset($attachment['delete_on_sent']) && $attachment['delete_on_sent'])
							{
								JFile::delete($attachment['path']);
							}
						}
					}
				}
