...
class plgSellaciousOrder extends JPlugin
{
	...
	protected function createMails($prefix, $order_id)
	{
		...
		$orderItems = $helper->order->getOrderItems($order_id);
		...
		if ($template->get('state'))
		{
			...
			if ($template->get('send_attachment'))
			{
				$filename = 'Invoice_' . $order->get('order_number');
				$options  = array(
					'type'     => 'f',
					'filename' => $filename,
				...
				$helper->order->renderInvoicePdf($order, '', $options);

				$orderFiles[] = array('path' => 'images/com_sellacious/orders/invoices/' . $filename . '.pdf' , 'name' => $filename . '.pdf');
			}

			$this->queue($ctx, $subject, $body, $recipients, $sender, $cc, $bcc, $replyTo, $orderFiles);
		}
	...
	protected function parseTemplate($order, &$subject, &$body, $seller = null)
	{
		...
		$dispatcher->trigger('onParseTemplate', array('com_sellacious.email.order', $order, &$replacements));
	...
	protected function getItems($order)
	{
		...
		foreach ($items as $item)
		{
			...
			else
			{
				...
				if ($helper->access->check('app.admin'))
				{
					$status_creator_role = JText::_('COM_SELLACIOUS_ORDER_USERTYPE_ADMIN');
				}
				elseif ($helper->access->check('app.manage'))
				{
			...
			$dispatcher = $helper->core->loadPlugins();
			$dispatcher->trigger('onParseTemplate', array('com_sellacious.email.order.product', $item, &$product));
