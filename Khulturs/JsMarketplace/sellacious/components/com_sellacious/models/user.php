...
class SellaciousModelUser extends SellaciousModelAdmin
{
			...
			foreach ($accounts as $type => $account)
			{
				if (!empty($account))
				{
					if ($type == 'seller')
					{
						...
						$account->geolocation = $this->helper->location->getGeoLocation('seller', $data->get('user_id'));
					}
					elseif ($type == 'client')
					{
	...
	public function save($data)
	{
		...
		if (!empty($seller['category_id']))
		{
			...
			unset($seller['geolocation']);

			$sId = $this->helper->user->addAccount($seller, 'seller');

			// Save geolocation
			$geolocation['address']     = $seller['store_location_address'];
			$geolocation['coordinates'] = $seller['store_location'];
			$this->helper->location->saveGeoLocation('seller', $seller['user_id'], $geolocation);
