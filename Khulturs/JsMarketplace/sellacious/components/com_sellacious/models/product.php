...
class SellaciousModelProduct extends SellaciousModelAdmin
{
	...
	protected function preprocessData($context, &$data, $group = 'content')
	{
		...
		if ($data->seller_uid)
		{
			...
			if (empty($data->seller))
			{
				...
				$data->seller->geolocation = $this->helper->location->getGeoLocation('product_sellers', (isset($data->seller->psx_id) ? $data->seller->psx_id : 0));
			}
		...
		if ($product_type)
		{
			...
			if ($seller_uid)
			{
				...
				if ($sellerEdit)
				{
					...
					if (!empty($data->seller->product_location))
					{
						$coordinates = explode(',', $data->seller->product_location);
						$form->setFieldAttribute('product_address', 'lat', $coordinates[0], 'seller');
						$form->setFieldAttribute('product_address', 'lng', $coordinates[1], 'seller');
					}
					elseif ($wasArray = is_array($data) && !empty($data['seller']['product_location']))
					{
						$coordinates = explode(',', $data['seller']['product_location']);
						$form->setFieldAttribute('product_address', 'lat', $coordinates[0], 'seller');
						$form->setFieldAttribute('product_address', 'lng', $coordinates[1], 'seller');
				...
				if ($shippingEdit && $product_type == 'physical')
				{
					...
					if (($shipRulesField && empty($rulesInput)) || $this->helper->config->get('flat_shipping', 1))
					{
			...
			if (!$editCols->get('location'))
			{
				...
				$form->removeField('zip', 'basic.geolocation');
				$form->removeField('sublocality', 'basic.geolocation');
				$form->removeField('locality', 'basic.geolocation');
				$form->removeField('city', 'basic.geolocation');
				$form->removeField('district', 'basic.geolocation');
				$form->removeField('state', 'basic.geolocation');
				$form->removeField('country', 'basic.geolocation');
			}
		}
		elseif (!$allowCreate)
		{
	 ...
	 * @since   1.0.0
	 */
	public function suggest($context, $key, $ids = null, $sellerUid = 0, $separate = false)
	{
		$items   = array();
		...
		if ($context == 'product-sku')
		{
			if ($separate)
			{
				$filters = array(
					'list.select'    => array(
						$sellerUid > 0 ? 'a.code AS id' : 'CASE WHEN a.seller_count > 1 THEN a.code ELSE a.product_id END AS id',
				...
				if ($key)
				{
					...
					$filters['list.where'][] = '(' .
						'a.product_title LIKE ' . $keyword . ' OR ' .
						'a.product_sku LIKE ' . $keyword . ' OR ' .
						'a.code LIKE ' . $keyword .
					')';
				}
				...
				if (isset($ids))
				{
					$pIds  = array_filter($ids, function ($id) { return $id > 0; });
					$codes = array_filter($ids, function ($id) { return $this->helper->product->parseCode($id); });

					if ($pIds)
					{
						$wOr[] = '(a.product_id IN (' . implode(', ', $pIds) . '))';
					}

					if ($codes)
					{
						$wOr[] = 'a.code IN (' . implode(', ', $this->_db->q($codes)) . ')';
					}

					if (isset($wOr))
					{
						$filters['list.where'][] = '(' . implode(' OR ', $wOr) . ')';
					}
				}

				if ($sellerUid > 0)
				{
					$filters['list.where'][] = 'a.seller_uid = ' . (int) $sellerUid;
				}

				$filters['list.where'][] = 'a.variant_id = 0';
			}
			elseif ($key)
			{
				$keyword = $this->_db->q('%' . $this->_db->escape($key) . '%', false);
		...
		elseif ($context == 'product')
		{
			if ($separate)
			{
				$filters = array(
					'list.select'    => array(
						'CASE WHEN a.seller_count > 1 THEN a.code ELSE a.product_id END AS id',
						'a.product_title as title',
					),
					'list.from'      => '#__sellacious_cache_products',
					'product_active' => 1,
