...
class SellaciousModelRatings extends SellaciousModelList
{
	...
	protected function getListQuery()
	{
		...
		COALESCE(c.seller_name, su.name) AS seller_company')
			->join('left', '#__sellacious_cache_products AS c ON c.product_id = a.product_id' . ($variantSeparate ? ' AND c.variant_id = a.variant_id' : ''));
		...
		if (!empty($search))
		{
			...
			elseif ($this->helper->product->parseCode($search, $product_id, $variant_id, $seller_uid))
			{
				...
				if ($seller_uid > 0)
				{
					$query->where('c.seller_uid = ' . (int) $seller_uid);
				}
			}
			else
			{
