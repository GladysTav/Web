...
class LanguagesControllerStrings extends SellaciousControllerAdmin
{
	...
	public function importExcel()
	{
		...
		if (!$this->helper->access->check('app.manage'))
		{
		...
		try
		{
			$uploader = new Uploader(array('xlsx', 'csv'));
			$files    = $uploader->select('jform.import_file', 1);
			$file     = reset($files);

			if (!$file)
			{
				throw new Exception('No file uploaded or the upload file is invalid.');
			}

			if ($file->extension === 'xlsx' && !class_exists('PhpOffice\PhpSpreadsheet\Spreadsheet', false))
			{
				throw new Exception(JText::_($this->text_prefix . '_IMPORT_LIBRARY_MISSING'));
			}

			/** @var  LanguagesModelStrings  $model */
			$model    = $this->getModel();
			$language = $model->getState('list.language');

			if (!$language)
			{
				throw new Exception(JText::_($this->text_prefix . '_TRANSLATE_AUTO_SELECT_LANGUAGE'));
			}
