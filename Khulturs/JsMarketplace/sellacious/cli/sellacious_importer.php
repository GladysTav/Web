...
use Sellacious\Cache\Product as ProductCache;
...
class SellaciousImporterCli extends SellaciousCliApplication
{
	...
	protected function doImport()
	{
		...
		$pks = $importer->getIndex('Product');

		$this->rebuildCache(array_values($pks));
	}

	/**
	 * Start the cache process
	 *
	 * @param   int[]  An array of imported (created/updated) product ids
	 *
	 * @return  void
	 *
	 * @throws  \Exception
	 ...
	 * @since   1.6.1
	 */
	protected function rebuildCache($pks)
	{
		...
		try
		{
			...
			foreach ($pks as $i => $pk)
			{
				...
				if (($i + 1) % 100 === 0)
				{
					$this->timer->log(sprintf('Rebuilding cache: %d of %d', $i + 1, $c), true);
				}
			}

			$this->timer->log(sprintf('Rebuilding cache: %d of %d', $c, $c), true);
