	...
	protected function addToolbar()
	{
		...
		if (count($this->items))
		{
			if ($this->_layout == 'bulk')
			{
				...
				elseif ($this->helper->access->checkAny(array('seller', 'seller.own'), 'product.edit.'))
				{
			...
			else
			{
				...
				if ($this->helper->access->checkAny(array('product.delete', 'product.delete.own')))
				{
					$gState->appendButton(new StandardButton('delete', 'COM_SELLACIOUS_PRODUCTS_SELLING_DELETE', $this->view_list . '.deleteListing', true));
				}
				...
				// We can allow direct 'delete' if trash not allowed but delete is permitted. Sure?
				elseif ($this->helper->access->checkAny(array('product.delete')))
				{
			...
			JToolBarHelper::custom($this->view_list . '.exportFiltered', 'export', 'export', 'COM_SELLACIOUS_PRODUCTS_EXPORT_PRODUCTS', false);
		}
	}
