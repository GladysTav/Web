...
class SellaciousModelCategory extends SellaciousModelAdmin
{
	...
	public function save($data)
	{
		...
		if ($pk > 0)
		{
			...
			$existing_translations = $this->helper->translation->getTranslations($pk, 'sellacious_categories');
		...
		try
		{
			...
			$_recordId   = $table->get('id');
			$_extensions = array('jpg', 'png', 'jpeg', 'gif');
			$_options    = ArrayHelper::getValue($data, 'icon', array(), 'array');
		...
		if ($isNew && $table->get('parent_id') == 1)
		{
			AccessHelper::setDefaultAccess($table->get('id'));
		}
	...
	protected function preprocessData($context, &$data, $group = 'content')
	{
		...
		if (is_object($data))
		{
			...
			if (in_array($cType, array('client', 'seller')))
			{
				$data->global_fields = $this->helper->field->getGlobalFields('user');
			}
			...
			elseif (strpos($cType, 'product/') !== false)
			{
				...
				$data->seller_commission = $rates;
				$data->global_fields     = $this->helper->field->getGlobalFields('product');
			}
			...
			$this->helper->translation->translateRecord($data, 'sellacious_categories');
	...
	protected function preprocessForm(JForm $form, $data, $group = 'sellacious')
	{
		...
		{
			$isPC = strpos($obj->type, 'product/') !== false;

			$form->loadFile('category/' . $obj->type, false);

			if (!$isPC || !$this->helper->config->get('product_compare'))
			{
			...
			if ($isPC)
			{
				$form->loadFile('category/product', false);
				$form->loadFile('category/product/config_prices', false);
			}
		...
		if (!$allowDuplicates)
		{
			$coreFields  = isset($obj->core_fields) ? (array) $obj->core_fields : array();
			$params      = new Registry(isset($obj->params) ? $obj->params : null);
			$customField = $params->get('product_unique_field');

			if (!is_numeric($customField) || in_array($customField, $coreFields))
			{
				$form->removeField('unique_field_custom_note', 'params');
			}
		}
		else
		{
			$form->removeField('shop_fieldgroup_product_unique_field', 'params');
			$form->removeField('product_unique_field', 'params');
			$form->removeField('unique_field_custom_note', 'params');
			$form->removeField('product_unique_field_scope', 'params');
		}
		...
		$form->setFieldAttribute('icon', 'recordId', $obj->id);
