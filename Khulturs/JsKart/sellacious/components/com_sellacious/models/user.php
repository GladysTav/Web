...
class SellaciousModelUser extends SellaciousModelAdmin
{
	...
	{
		return $this->helper->access->check('user.edit.state');
	}
	...
	protected function preprocessForm(JForm $form, $data, $group = 'sellacious')
	{
		...
		if ($catIdC)
		{
			...
			{
				if (!$config->get('allow_client_authorised_users'))
				{
					$form->removeField('authorised', 'client');
				}

				if (!$config->get('allow_credit_limit'))
				{
	...
	public function save($data)
	{
		...
		if (!empty($seller['category_id']))
		{
			...
			$user->set('seller_category_id', $seller['category_id']);
		}
		else
		{
			// Remove from existing
	...
	protected function getVisible($catid)
	{
		...
		return $params ? $params->get('admin') : null;
	}
	...
	{
		$usergroup = ArrayHelper::getValue($commands, 'usergroup', 0, 'int');
		$category  = ArrayHelper::getValue($commands, 'category', 0, 'int');
		$filter    = ArrayHelper::getValue($commands, 'filter', 0, 'int');
		$action    = ArrayHelper::getValue($commands, 'action', 0, 'int');

		if ($action != 1 && $action != -1)
		{
			throw new Exception(JText::_('COM_SELLACIOUS_USER_BATCH_ERROR_INVALID_ACTION'));
		}

		$pks = JAccess::getUsersByGroup($usergroup);

		if (count($pks))
		{
			$roles = UserHelper::getRoles();
			$type  = $this->helper->category->loadResult(array('id' => (int) $category, 'list.select' => 'a.type'));

			if (!array_key_exists($type, $roles))
			{
				throw new Exception(JText::_('COM_SELLACIOUS_USER_BATCH_ERROR_INVALID_CATEGORY'));
			}

			if ($action == -1)
			{
				// Only existing can be removed, intersect to optimize
				$eks = UserHelper::getUsersByGroup($category, false);
				$pks = array_intersect($pks, $eks);
			}
			elseif ($filter == 1)
			{
				// Filter for Add action: 1 = Unassigned only, 2 = All
				$eks = UserHelper::getUsersByRole($type, false);
				$pks = array_diff($pks, $eks);
			}
			...
			foreach ($pks as $i => $pk)
			{
				if ($action == 1)
				{
					$this->helper->profile->create($pk);
				}

				$table = $this->getTable($type);
				$table->load(array('user_id' => $pk));

				if (!$table->get('id'))
				{
					$table->set('user_id', $pk);
					$table->set('state', 1);
				}

				$table->set('category_id', $action == 1 ? $category : 0);

				$table->check();
				$table->store();

				$this->state->set('user.batch.count', $i + 1);
			}
		}
	...
	protected function addUserMail($template, $user)
	{
		...
		$emailParams = $this->helper->config->getEmailParams();
