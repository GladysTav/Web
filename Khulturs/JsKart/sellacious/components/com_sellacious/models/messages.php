...
class SellaciousModelMessages extends SellaciousModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select('COUNT(DISTINCT c3.id) AS children, IFNULL(MAX(c3.date_sent), a.date_sent) AS last_update')
		...
		// Only load first message of all threads
		$query->where('a.level = 1 AND a.parent_id = 1');
		...
		$ordering = $this->state->get('list.fullordering', 'last_update DESC');
