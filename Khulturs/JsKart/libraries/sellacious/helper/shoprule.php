...
use Sellacious\Shoprule\ShopruleHandler;
...
class SellaciousHelperShoprule extends SellaciousHelperBase
{
	...
	public function getHandlers($skipDefault = false)
	{
		if (!$this->defaultHandlers)
		{
			...
			$this->defaultHandlers['slabs.quantity'] = new ShopruleHandler('slabs.quantity', 'COM_SELLACIOUS_SHOPRULE_METHOD_QUANTITY_SLAB');
			$this->defaultHandlers['slabs.price']    = new ShopruleHandler('slabs.price', 'COM_SELLACIOUS_SHOPRULE_METHOD_PRICE_SLAB');
		}
		...
		if (!$this->handlers)
		{
			...
			$dispatcher->trigger('onCollectHandlers', array('com_sellacious.shoprule', &$handlers));

			foreach ($handlers as $name => $handler)
			{
				if ($handler instanceof ShopruleHandler)
				{
				...
				elseif (is_string($handler))
				{
					...
					$this->handlers[$name] = new ShopruleHandler($name, $handler);
				}
			}
	...
	protected function getRules()
	{
		...
		$rules   = array();
		$filters = array(
			'list.select'    => 'a.id, a.title, a.parent_id, a.level, a.type, a.method_name, a.sum_method, a.params, a.amount, a.apply_rule_on_price_display, a.apply_rule_on_list_price, a.apply_on_all_products',
			'list.where'     => array(
				'a.state = 1',
				'a.level > 0',
	...
	public function toProduct($item, $use_cart = false, $check_sync = false)
	{
		...
		// sales_price, is_fallback, client_catid
		...
		foreach ($rules as $rule)
		{
			...
			if ($method_name != '*')
			{
				...
				$handler  = ArrayHelper::getValue($handlers, $method_name);

				if ($handler instanceof ShopruleHandler)
				{
					if ($handler->name == 'slabs.quantity')
					{
						$slabValue = isset($item->quantity) ? $item->quantity : 1;
						$slab      = $this->matchSlab($rule_id, $slabValue, $origin);

						if ($slab)
						{
							$rule->set('percent', $slab->unit == '%');
							$rule->set('amount', $slab->price);
						}
						else
						{
							continue;
		...
		foreach ($values as $value)
		{
			if ($value->type == 'tax')
			{
				...
				if ($value->apply_rule_on_list_price && $check_sync && $value->sum_method == 2)
				{
					$nett_list_price_tax += $value->change;
				}
			}
			elseif ($value->type == 'discount')
			{
				...
				if ($value->apply_rule_on_list_price && $check_sync && $value->sum_method == 2)
				{
					$nett_list_price_discount += $value->change;
				}
			}
		}
		...
		$item->list_price_final = max(0, $item->list_price + $nett_list_price_tax - $nett_list_price_discount);
	...
	public function toCart($totals, $cart)
	{
		...
		foreach ($rules as $rule)
		{
			...
			{
				$handlers = $this->helper->shopRule->getHandlers();
				$handler  = ArrayHelper::getValue($handlers, $rule->get('method_name'));
				...
				if ($handler instanceof ShopruleHandler)
				{
					...
					elseif ($handler->name == 'slabs.price')
					{
						$slabValue = $totals->get('cart.sub_total');
						$slab      = $this->matchSlab($rule_id, $slabValue, $origin);
	...
	/**
	 * Apply shoprules to Shipping
	 *
	 * @param   ShippingQuote  $shipping  The shipping object to be processed
	 *
	 * @return  array
	 *
	 * @throws  \Exception
	 ...
	 * @since   1.7.1
	 */
	public function toShipping($shipping)
	{
		$full_stack = array();
		$values     = array();
		$rules      = $this->getRules();
		...
		foreach ($rules as $rule)
		{
			...
			if (isset($base))
			{
				...
				// We are setting cart shoprules in-effective here
				if ($rule->get('sum_method') == SellaciousHelperShoprule::SUM_FOR_SHIPPING)
				{
					try
					{
						$plugin_args = array('com_sellacious.shoprule.shipment', &$rule, $shipping);
						$responses   = $dispatcher->trigger('onValidateShipmentShoprule', $plugin_args);
					}
					catch (Exception $e)
					{
						// Currently, rules (may) get applied on exception. Should we handle this exception?
		...
		$dispatcher->trigger('onAfterShipmentShoprules', array('com_sellacious.cart', $shipping, &$values));
		...
		$shipping->tax_amount      = $nett_tax;
		$shipping->discount_amount = $nett_discount;
		$shipping->sub_total       = max(0, $shipping->total + $nett_tax - $nett_discount);
	 ...
	 * @since   1.6.0
	 *
	 * @deprecated   Update this to use pricing-type and mention where it is used. Otherwise this will be removed
	 */
	public function getDiscountFilter($discountId)
	{
		$shopRule = $this->helper->shopRule->loadObject(array('id' => $discountId));
		$params   = new Registry($shopRule->params);
		$where    = array();

		if ($params->get('product'))
		{
			$prIds = (array) $params->get('product.products');
			...
			$mfrIds = (array) $params->get('product.manufacturer');
			...
			$catIds = (array) $params->get('product.categories');
			...
			if ($params->get('amountfilter.min_amount') > 0.01)
			{
				$where[] = 'a.product_price >= ' . $params->get('amountfilter.min_amount');
			}

			if ($params->get('amountfilter.max_amount') > 0.01)
			{
				$where[] = 'a.product_price <= ' . $params->get('amountfilter.max_amount');
			}
		}
	...
	/**
	 * Remove any existing slabs from the database for the given shop rule
	 *
	...
	public function clearSlabs($ruleId)
	{
		...
		$query->delete('#__sellacious_shoprule_slabs')->where('rule_id = ' . (int) $ruleId);
	...
	/**
	 * Add a new slabs to the given shop rule
	 *
	 * @param   int       $ruleId  The rule id
	...
	public function addSlab($ruleId, $slab)
	{
		...
		return $this->db->insertObject('#__sellacious_shoprule_slabs', $slab);
	}

	/**
	 * Get a list of all slabs for the given shop rule
	 *
	 * @param   int  $ruleId  The rule id
	 *
	 * @return  stdClass[]
	...
	{
		$query = $this->db->getQuery(true);
		$query->select('*')->from('#__sellacious_shoprule_slabs')->where('rule_id = ' . (int) $ruleId);

		return (array) $this->db->setQuery($query)->loadObjectList();
	}

	/**
	 * Method to find a suitable slab for the given cart item(s) shop rule
	 *
	 * @param   int       $ruleId
	 * @param   int       $value
	 ...
	 * @since   2.0.0
	 */
	public function matchSlab($ruleId, $value, $origin = null)
	{
		$cart    = $this->helper->cart->getCart();
		$address = $cart->getBillTo(true);

		$query = $this->db->getQuery(true);
		$query->select('a.*')->from($this->db->qn('#__sellacious_shoprule_slabs', 'a'))->where('rule_id = ' . (int) $ruleId);
