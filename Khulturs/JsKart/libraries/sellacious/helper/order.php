...
class SellaciousHelperOrder extends SellaciousHelperBase
{
	 ...
	 * @since   1.0.0
	 */
	public function setStatusByType($context, $statusType, $order_id, $item_uid, $recursive = true, $core = false, $notes = '', $seller_uid = 0)
	{
		$statusId = $this->getStatusId($statusType, $core, $context);

		if ($statusId)
		{
			// ['order_id', 'item_uid', 'seller_uid', 'status', 'notes', 'shipment', 'params']
			$arr = array(
				'order_id'   => $order_id,
				'item_uid'   => $item_uid,
	...
	/**
	 * Short hand method to set order/seller status
	 *
	 * @param   string  $context
	 * @param   string  $statusType
	 * @param   int     $order_id
	 ...
	 * @since   2.0.0
	 */
	public function setStatusForSellers($context, $statusType, $order_id, $core = false, $notes = '')
	{
		$sellers = $this->getSellers($order_id);

		foreach ($sellers as $seller)
		{
			$this->setStatusByType($context, $statusType, $order_id, '', false, $core, $notes, $seller->seller_uid);
		}
	}
	...
	public function setStatus(array $record, $recursive = true)
	{
		...
		$seller_uid = ArrayHelper::getValue($record, 'seller_uid', null);
		$item_uid   = ArrayHelper::getValue($record, 'item_uid', null);

		// Whether order status needs to be updated seller wise or not
		$status_change = $this->helper->config->get('order_status_change', 0);
		$seller_wise   = !$item_uid && $status_change == 1;

		$old = $this->getStatus($order_id, $item_uid, $seller_uid);
		...
		if ($saved)
		{
			...
			if ($item_uid)
			{
				$query->where('item_uid = ' . $this->db->q($item_uid));
			}

			if ($seller_uid)
			{
				$query->where('seller_uid = ' . $this->db->q($seller_uid));
			}
		...
		$new = $this->getStatus($order_id, $item_uid, $seller_uid);
		...
		elseif ($seller_wise && $seller_uid > 0)
		{
			// We must do the seller-wise stock handling if required.
			$this->handleStock($order_id, $item_uid, $seller_uid);
		}

		// Early exit if nothing more to do
		// We don't want to run recursive ever for seller wise status change
		if ($recursive && !$seller_wise)
		{
			if ($item_uid)
			{
		...
		if ($new->s_type == 'approved')
		{
			if ($new->s_context == 'order')
			{
				...
				$this->helper->payment->markPaid('order', $order_id);
		...
		$status_invoice = $this->helper->config->get('invoice_order_status');

		// Generate invoice on order status
		if ($status_invoice != '' && $status_invoice == $record['status'])
		{
			$this->helper->order->createInvoices($order_id);
		}
	 ...
	 * @param   int     $seller_uid  Seller user id within order
	 *
	 * @return  stdClass
	 *
	 * @throws  Exception
	 ...
	 * @since   1.0.0
	 */
	public function getStatus($order_id, $item_uid = null, $seller_uid = null)
	{
		$query = $this->getStatusQuery($order_id, $item_uid, $seller_uid);
		...
		try
		{
			...
			if ($status)
			{
				...
				$this->helper->translation->translateValue($status->status, 'sellacious_status', 'title', $status->s_title);
			}
		}
		catch (Exception $e)
		{
		...
		if (!$status)
		{
			...
			$status = new stdClass;
			$props  = array('id', 'order_id', 'seller_uid', 'item_uid', 'status', 'notes', 'customer_notes', 'shipment', 'created',
	 ...
	 * @since   1.2.0
	 */
	protected function getStatusQuery($order_id, $item_uid = null, $seller_uid = null)
	{
		$query = $this->db->getQuery(true);
		...
		elseif (isset($seller_uid))
		{
			// Check seller user id
			$query->where('a.seller_uid = ' . $this->db->q($seller_uid));
		}
	...
	public function getStatuses($context, $old = null, $id_only = false, $access = false)
	{
		...
		try
		{
			...
			$statuses = $this->loadObjectList($filters);
			...
			if ($access)
			{
				...
				{
					$usergroups = isset($status->usergroups) ? json_decode($status->usergroups) : array();

					if (!empty($usergroups) && empty(array_intersect($usergroups, $groups)) && !$user->authorise('app.admin'))
					{
						unset($statuses[$key]);
					}

					$this->helper->translation->translateRecord($status, 'sellacious_status');
				}
			}

			if ($id_only)
			{
				$statuses = ArrayHelper::getColumn($statuses, 'id');
			}
		}
		catch (Exception $e)
		{
	 ...
	 * @param   int  $itemUid    The uid of the order item (PnVnSn)
	 * @param   int  $sellerUid  The seller user id of the order
	 *
	 * @return  void
	 *
	 * @throws  Exception
	 ...
	 * @since   1.0.0
	 */
	public function handleStock($orderId, $itemUid, $sellerUid = null)
	{
		if ($sellerUid > 0)
		{
			$history = $this->getStatusLog($orderId, null, $sellerUid);
		}
		else
		{
			$history = $this->getStatusLog($orderId, $itemUid);
		}
		...
		// If seller id is provided, update stock for each item in seller
		if ($sellerUid > 0)
		{
			$filters['seller_uid'] = $sellerUid;

			$orderItems = $this->loadObjectList($filters);

			foreach ($orderItems as $oi)
			{
				$this->updateStock($oi, $change);
			}
		}
		else
		{
			$filters['item_uid'] = $itemUid;
	...
	/**
	 * Method to update seller stocks
	 *
	 * @param   stdClass  $orderItem     The order item object
	 * @param   array     $stockChanges  Array changes to make in stock
	 *
	 * @since   2.0.0
	 */
	protected function updateStock($orderItem, $stockChanges)
	{
		...
		if ($orderItem->variant_id)
		{
			...
			$table->load(array('variant_id' => $orderItem->variant_id, 'seller_uid' => $orderItem->seller_uid));
		}
		else
		{
			$table = $this->getTable('ProductSeller');

			$table->load(array('product_id' => $orderItem->product_id, 'seller_uid' => $orderItem->seller_uid));
		}
		...
		if ($table->get('id'))
		{
			...
			$c_stock += $stockChanges[0] * $orderItem->quantity;
			$r_stock += $stockChanges[1] * $orderItem->quantity;
			$o_stock += $stockChanges[2] * $orderItem->quantity;
	 ...
	 * @since   1.0.0
	 */
	public function getStatusLog($order_id, $item_uid = null, $seller_uid = null)
	{
		$db    = $this->db;
		$query = $this->getStatusQuery($order_id, $item_uid, $seller_uid);
	...
	public function checkEProductDelivery($delivery_id, $file_id = null)
	{
		...
		if ($file_ids && (int) $delivery->download_limit > 0)
		{
			...
			{
				$count = $this->helper->media->getDownloadCount(null, $file_id, 'media', $delivery->id);

				if ($count < (int) $delivery->download_limit)
				{
	 ...
	 * @since   1.0.0
	 *
	 * @deprecated   Use $helper->media->addDownloadEntry() directly
	 */
	public function logDownload($delivery_id, $file)
	{
		$this->helper->media->addDownloadEntry($file, JFactory::getUser()->id, $delivery_id, null);
	}
	 ...
	 * @since   1.4.4
	 *
	 * @deprecated   Use payment helper directly
	 */
	public function isPaid($orderId)
	{
		return $this->helper->payment->isPaid('order', $orderId);
	}
	 ...
	 * @since   2.0.0
	 */
	public function hasBillingAddress($order_id)
	{
		$filter = array(
			'id'          => $order_id,
			'list.select' => array(
	 ...
	 * @since   2.0.0
	 */
	public function checkAddressPending($orderId)
	{
		return !$this->hasBillingAddress($orderId) || !$this->hasShippingAddress($orderId);
	}
	 ...
	 * @since   1.7.0
	 */
	public function createInvoices($order_id)
	{
		...
		if ($seller_separate_invoice == 1)
		{
			...
			foreach ($sellers as $seller)
			{
				$invoice = new Invoice($order_id, $seller->seller_uid);
	...
	public function getInvoiceHtml($order)
	{
		...
		if ($seller_separate_invoice == 1)
		{
			...
			foreach ($sellers as $seller)
			{
				...
				$dispatcher->trigger('onParseViewTemplate', array('com_sellacious.order.pdf', $invoice, &$sellerHtml, array('seller_uid' => $seller->seller_uid)));

				if ($count < count($sellers))
				{
					$sellerHtml .= '<div style="page-break-before: always;"></div>';
				}
		...
		{
			$html 		= '';
			$dispatcher->trigger('onParseViewTemplate', array('com_sellacious.order.pdf', $order, &$html));
		}
	...
	/**
	 * Render pdf from the html layout using tcPDF or from existing PDF
	 *
	 * @param   Registry  $order     Object with order details
	 * @param   string    $content   Content for Invoice PDF
	 * @param   array     $options   Extra options
	 * @param   bool      $viewOnly  PDF only for view, do not create if it doesn't exists
	 *
	 * @throws  \Exception
	 *
	 * @since   1.7.0
	 */
	public function renderInvoicePdf($order, $content = '', $options = array(), $viewOnly = false)
	{
		jimport('joomla.filesystem.folder');

		$app      = JFactory::getApplication();
		$filename = ArrayHelper::getValue($options, 'filename', 'Invoice_' . $order->get('order_number'));
		$path     = ArrayHelper::getValue($options, 'path', $filename . '.pdf');
		$type     = strtoupper(ArrayHelper::getValue($options, 'type', 'I'));

		try
		{
			if ($viewOnly)
			{
				$this->createInvoicePdf($order, $content, $options, $viewOnly);
			}
			elseif (JFile::exists($path))
			{
				// If PDF file exists, then just render it
				$disposition = $type == 'I' ? 'inline' : 'attachment';
			...
			{
				// If PDF File does not exist, create it
				$pdfOptions         = $options;
				$pdfOptions['type'] = 'f';

				if (empty($content))
				{
					$content = $this->getInvoiceHtml($order);
				}

				$this->createInvoicePdf($order, $content, $pdfOptions, $viewOnly);
				$this->renderInvoicePdf($order, $content, $options);
			}
		}
		catch (Exception $e)
		{
			...
			throw new Exception(JText::sprintf('COM_SELLACIOUS_INVOICE_RENDER_ERROR', $e->getMessage()));
		}
	...
	/**
	 * Create pdf from the html layout using tcPDF
	 *
	 * @param   Registry  $order     Object with order details
	 * @param   string    $content   Content for Invoice PDF
	 * @param   array     $options   Extra options
	 ...
	 * @since   1.7.0
	 */
	public function createInvoicePdf($order, $content = '', $options = array(), $viewOnly = false)
	{
		$filename = ArrayHelper::getValue($options, 'filename', 'Invoice_' . $order->get('order_number'));
		$path     = ArrayHelper::getValue($options, 'path', $filename . '.pdf');
		$type     = strtoupper(ArrayHelper::getValue($options, 'type', 'I'));
		$title    = ArrayHelper::getValue($options, 'title', 'Invoice_' . $order->get('order_number'));
		$subject  = ArrayHelper::getValue($options, 'subject', 'Invoice_' . $order->get('order_number'));

		if (!JFolder::exists(JPATH_SITE . '/images/com_sellacious/orders/invoices'))
		{
			JFolder::create(JPATH_SITE . '/images/com_sellacious/orders/invoices', 0755);
		}

		$doc = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		$doc->SetCreator(JFactory::getConfig()->get('sitename'));
		$doc->SetAuthor(JFactory::getConfig()->get('sitename'));
		$doc->SetTitle($title);
		...
		$doc->SetMargins(10, 10, 10, true);
		$doc->SetAutoPageBreak(true, 10);
		$doc->setImageScale(PDF_IMAGE_SCALE_RATIO);
		$doc->SetDisplayMode('fullpage', 'SinglePage', 'UseNone');
		$doc->SetTextColor(119, 119, 119);
		$doc->SetDrawColor(119, 119, 119, false, '');
		...
		$doc->Output($path, $type);
	}

	/**
	 * Get a map for the stock changes for given order item's last status change
	 *
	 * @param   int     $orderId
	 ...
	 * @since   2.0.0
	 */
	public function getStatusStockChange($orderId, $itemUid)
	{
		$history = $this->getStatusLog($orderId, $itemUid);
		...
		while ($prev = array_shift($history))
		{
			if (in_array($prev->s_stock, array('A', 'R', 'O')))
			{
				$o_stock = $prev->s_stock;

				break;
			}
		}

		// [OLD/NEW] => [A=Stock, R=Reserved, O=Sold]
		$map = array(
			'-A' => array( '0',  '0',  '0'),
			'-R' => array('-1', '+1', '0'),
			'-O' => array('-1', '0', '+1'),
		...
		return ArrayHelper::getValue($map, $o_stock . $recent->s_stock);
	}

	/**
	 * Check whether the order has shippable product
	 *
	 * @param   int  $orderId
	 *
	 * @return  bool
	 ...
	 * @since   2.0.0
	 */
	public function hasShippable($orderId)
	{
		$items = $this->getOrderItems($orderId, 'id', array('a.product_type = ' . $this->db->q('electronic')));

		return count($items) == 0;
	}

	/**
	 * Set addresses to an existing order
	 *
	 * @param   int  $orderId
	 * @param   int  $billing
	 * @param   int  $shipping
	 ...
	 * @since   2.0.0
	 */
	public function setAddresses($orderId, $billing, $shipping)
	{
		$order = new stdClass;

		if ($billing)
		{
			$addr   = $this->helper->user->getAddressById($billing);

			if ($addr)
			{
				$address = new Registry($addr);

				$address->set('country_title', $this->helper->location->loadResult(array('list.select' => 'a.title', 'id' => $addr->country)));
				$address->set('state_title', $this->helper->location->loadResult(array('list.select' => 'a.title', 'id' => $addr->state_loc)));
				$address->set('district_title', $this->helper->location->loadResult(array('list.select' => 'a.title', 'id' => $addr->district)));

				$order->bt_name        = $address->get('name');
				$order->bt_address     = $address->get('address');
				$order->bt_landmark    = $address->get('landmark');
				$order->bt_district    = $address->get('district_title');
				$order->bt_state       = $address->get('state_title');
				$order->bt_zip         = $address->get('zip');
				$order->bt_country     = $address->get('country_title');
				$order->bt_mobile      = $address->get('mobile');
				$order->bt_company     = $address->get('company');
				$order->bt_po_box      = $address->get('po_box');
				$order->bt_residential = $address->get('residential');
			}
		}

		if ($shipping)
		{
			$addr = $this->helper->user->getAddressById($shipping);
			...
			if ($addr)
			{
				...
				$order->st_name        = $address->get('name');
				$order->st_address     = $address->get('address');
				$order->st_landmark    = $address->get('landmark');
				$order->st_district    = $address->get('district_title');
				$order->st_state       = $address->get('state_title');
				$order->st_zip         = $address->get('zip');
				$order->st_country     = $address->get('country_title');
				$order->st_mobile      = $address->get('mobile');
				$order->st_company     = $address->get('company');
				$order->st_po_box      = $address->get('po_box');
				$order->st_residential = $address->get('residential');
				$order->bt_same_st     = $billing == $shipping;
			}
		}
