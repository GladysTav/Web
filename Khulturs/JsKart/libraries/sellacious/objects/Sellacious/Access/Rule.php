...
class Rule
{
	...
	public function __construct($identities)
	{
		...
		$this->replaceIdentities($identities);
	}
	 ...
	 * @since   2.0.0
	 */
	public function replaceIdentities($identities)
	{
		if ($identities instanceof Rule)
		{
			$identities = $identities->getData();
		...
		if (is_array($identities))
		{
			foreach ($identities as $identity => $allow)
			{
				$this->replaceIdentity($identity, $allow);
			}
		}
	}

	/**
	 * Replace the values for an identity.
	 *
	 * @param   integer  $identity  The identity.
	 * @param   mixed    $allow     The value for the identity (true == allow, false == deny, '' == inherit).
	 *
	 * @return  void
	 *
	 * @since   2.0.0
	 */
	public function replaceIdentity($identity, $allow)
	{
		$identity = (int) $identity;
