...
{
	/**
	 * The database table name that will hold the permission rules for sellacious
	 *
	 * @var    array
	 *
	 * @since  2.0.0
	 */
	protected static $assetTable = '#__sellacious_permissions';
	...
	/**
	 * Array of rules for the asset's explicit rules
	 *
	 * @var    array
	 *
	 * @since  2.0.0
	 */
	protected static $assetOwnRules = array();
	 ...
	 * @param   boolean         $preload   Indicates whether pre-loading should be used.
	 *
	...
	public static function check($userId, $action, $assetKey = null, $preload = true)
	{
		...
		if (!isset(self::$identities[$userId]))
		{
			...
			self::$identities[$userId] = UserHelper::getGroupsByUser($userId);
	...
	public static function preload($assetTypes = 'components', $reload = false)
	{
		...
		// If we get to this point, this is a regular asset type and we'll proceed with the pre-loading process.
	...
	protected static function preloadPermissions($assetType, $reload = false)
	{
		...
		$db         = \JFactory::getDbo();
		$extraQuery = $db->qn('a.name') . ' = ' . $db->q($extensionName) . ' OR ' . $db->qn('a.parent_id') . ' = 0';

		// Get a fresh query object.
		$query = $db->getQuery(true)
			->select($db->qn(array('a.id', 'a.name', 'a.rules', 'a.parent_id')))
			->from($db->qn(static::$assetTable, 'a'))
			->where($db->qn('a.name') . ' LIKE ' . $db->q($extensionName . '.%') . ' OR ' . $extraQuery);
	...
	protected static function preloadComponents()
	{
		...
		$query = $db->getQuery(true)
			->select($db->qn(array('a.id', 'a.name', 'a.rules', 'a.parent_id')))
			->from($db->qn(static::$assetTable, 'a'))
			->where($db->qn('a.name') . ' IN (' . implode(', ', $db->quote($components)) . ')');
	...
	public static function checkGroup($groupId, $action, $assetKey = null, $preload = true)
	{
		...
		$identity = self::getGroupPath($groupId);

		return $rules->allow($action, $identity);
	}
	...
	protected static function getGroupPath($groupId)
	{
		...
		return $group ? $group->hierarchy : array();
	}
	...
	public static function getAssetRules($assetKey, $recursive = false, $recursiveParentAsset = true, $preload = true)
	{
		...
		if ($recursive)
		{
			$query->join('LEFT', $db->qn(static::$assetTable, 'b') . ' ON b.lft <= a.lft AND b.rgt >= a.rgt')
				->order($db->qn('b.lft'));
		}
	...
	/**
	 * Method to return the Rules object for an assets explicit rules set.
	 *
	 * @param   integer|string  $assetKey  The asset key (asset id or asset name). null fallback to root asset.
	 *
	 * @return  Rules  Rules object for the asset.
	 ...
	 * @since   2.0.0
	 */
	public static function getAssetOwnRules($assetKey)
	{
		$assetKey = self::cleanAssetKey($assetKey);
		...
		if (!isset(self::$assetOwnRules[$assetKey]))
		{
			$db    = \JFactory::getDbo();
			$query = $db->getQuery(true);
			...
			$query->select('a.id, a.name, a.rules')
				->from($db->qn(static::$assetTable, 'a'))
				->where((is_numeric($assetKey) ? 'a.id' : 'a.name') . ' = ' . $db->q($assetKey));
			...
			$rules = new Rules($asset->rules);

			self::$assetOwnRules[$asset->id]   = $rules;
			self::$assetOwnRules[$asset->name] = $rules;
		}

		return self::$assetOwnRules[$assetKey];
	}
	...
	protected static function cleanAssetKey($assetKey = null)
	{
		...
		// No preload. Return root asset id from Assets (we skip querying from db and return hardcoded).
		return 1;
	}
		...
		if (!isset($loaded[$assetKey]))
		{
			...
			else
			{
				...
				else
				{
					...
					$query->select('id')->from(static::$assetTable)->where('name' . ' = ' . $db->q($assetKey));

					$loaded[$assetKey] = $db->setQuery($query)->loadResult();
				}
			}
		}
	...
	protected static function getAssetName($assetKey)
	{
		...
		if (!isset($loaded[$assetKey]))
		{
			...
			else
			{
				...
				$query->select('name')->from(static::$assetTable)->where('id' . ' = ' . $db->q($assetKey));
	...
	{
		$file  = JPATH_BASE . '/components/' . $component . '/access.xml';
		$xpath = "/access/section[@name='" . $section . "']/";

		$actions = self::getActionsFromFile($file, $xpath);
