...
class Internal extends Item
{
	...
	{
		if ($this->cart->getOptions()->get('itemised_shipping') == \SellaciousHelperShipping::SHIPPING_SELECTION_CART || $this->cart->getOptions()->get('itemised_shipping') == \SellaciousHelperShipping::SHIPPING_SELECTION_SELLER)
		{
	...
	public function check()
	{
		...
		$dispatcher->trigger('onValidateCartItem', array('com_sellacious.cart', &$this, &$errors));
		...
		$coqData = CheckoutQuestionsFormHelper::getData('cart_summary', $this->cart, $this->getUid());
		$coqForm = $this->helper->cart->getCheckoutForm(true, 'cart_summary', $this->getUid());

		if ($coqForm && !CheckoutQuestionsFormHelper::validateCheckoutForm($coqForm, $coqData, 'cart_summary'))
		{
	...
	{
		$images = $this->product->getImages(true, true);
	...
	protected function build()
	{
		...
		$sellerAttributes = $product->getSellerAttributes($this->sellerUid);

		$sellerAttributes->type = $product->get('type');

		$this->helper->product->setReturnExchange($sellerAttributes);
		unset($sellerAttributes->type);

		$product->bind($sellerAttributes);
