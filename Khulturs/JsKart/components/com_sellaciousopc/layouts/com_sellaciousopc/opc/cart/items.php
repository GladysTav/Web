	...
	foreach ($items as $uid => $cartItem)
	{
		...
		<tr>
			...
			<td class="cart-item">
				...
				<?php endif; ?>
				<br><div><?php echo JText::_('COM_SELLACIOUSOPC_CART_ITEM_SOLD_BY'); ?> <span><?php echo $cartItem->getProperty('seller_store') ? $cartItem->getProperty('seller_store') : ($cartItem->getProperty('seller_name') ? $cartItem->getProperty('seller_name') : ($cartItem->getProperty('seller_company') ? $cartItem->getProperty('seller_company') : $cartItem->getProperty('seller_username'))); ?></span></div>
		...
		<tr>
			<td colspan="5">
				...
				<a class="btn btn-small pull-left btn-refresh btn-default margin-5"><?php
					echo JText::_('COM_SELLACIOUSOPC_CART_BTN_REFRESH_CART_LABEL') ?> <i class="fa fa-sync"></i> </a>
				<a class="btn btn-small pull-left btn-clear-cart btn-warning margin-5"><?php
					echo JText::_('COM_SELLACIOUSOPC_CART_BTN_CLEAR_CART_LABEL') ?> <i class="fa fa-trash-alt"></i> </a>
				<a class="btn btn-small pull-left btn-close btn-default margin-5"><?php
					echo JText::_('COM_SELLACIOUSOPC_CART_BTN_CLOSE_CART_LABEL') ?> <i class="fa fa-times"></i> </a>
			</td>
		</tr>
	</tfoot>
</table>
