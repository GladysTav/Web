...
JS;
	...
	public function onTemplatePreview($context, $templateContext, &$body)
	{
		if ($context == 'com_sellacious.viewtemplate')
		{
			...
			$replacements = array(
				...
				'invoice_date'              => JHtml::_('date', 'now', 'F d, Y'),
				'order_id'                  => '5',
				'order_number'              => '190005',
	...
	public function parseTemplate($order, &$body, $context, $options = array())
	{
		...
		$invoice      = new Invoice($orderId, $sellerUid);
		$orderInvoice = $invoice->getItem();
		...
		$sellers              = $items ? ArrayHelper::getColumn($items, 'seller_company') : array();
		...
		$replacements = array(
			...
			'invoice_date'              => JHtml::_('date', $orderInvoice->get('created', $order->get('created')), 'F d, Y'),
			'order_id'                  => $orderId,
	...
	protected function getItems($order)
	{
		...
		foreach ($items as $item)
		{
			...
			if (empty($log))
			{
				$log = $helper->order->getStatusLog($orderId, null, $item->seller_uid);
			}
