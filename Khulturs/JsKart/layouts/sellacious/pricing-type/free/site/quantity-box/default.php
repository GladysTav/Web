...
$display_stock    = $helper->config->get('display_stock_in_product_detail');
...
if ($disable_checkout == 0 && ($showAddToCart || $showBuyNow) && (!$reqLogin || !$me->guest))
{
	if ($checkStock || $item->get('disable_stock', 0) == 1)
	{
		...
		if ($display_stock == 1)
		{
			?><div class="fa fa-check-circle"></div><?php
	...
	else
	{
		?>
		<div class="qtyarea">
			<button disabled="disabled" class="disabled ctech-btn lbl-no-stock ctech-btn-default hasTooltip outofstock"
				data-original-title="<?php echo JText::_('COM_SELLACIOUS_PRODUCT_OUT_OF_STOCK'); ?>">
