<div :class="[params.widthClasses]" class="product-list-block product-block-jskart">
		...
		<div class="product-images-container" data-rollover="container">
			...
			<div class="product-stock-info" v-if="params.context.includes('wishlist')">
				<span :class="(product.stock_capacity > 0 || product.disable_stock == 1) ? 'ctech-badge-info' : 'ctech-badge-danger'"
			  		class="ctech-badge">{{ (product.stock_capacity > 0 || product.disable_stock == 1) ? 'COM_SELLACIOUS_PRODUCTS_IN_STOCK' : 'COM_SELLACIOUS_PRODUCTS_OUT_OF_STOCK' | t }}</span>
			</div>
		</div>
		<div class="product-information">
			<div class="product-info">
				<div class="product-title"><a :href="product.url"
					:class="params.product_detail_page === '0' ? 'not-clickable' : ''">{{ product.product_title }}<span class="product-variant-title"
				...
				<template v-if="(product.compare_allow && params.compare_pages.includes('products')) || params.show_modal.includes('products') || (product.hasOwnProperty('rendered_attr'))">
					<div class="product-action-buttons">
						<template v-if="product.price_display == 0 && (params.login_to_see_price || user.guest)">
							<template v-if="product.stock_capacity">
								<template v-if="product.hasOwnProperty('rendered_attr') && product.rendered_attr.length > 0">
									<a class="ctech-btn ctech-btn-primary ctech-text-white btn-quick-view hasTooltip" :title="t('COM_SELLACIOUS_PRODUCTS_DELIVERY_SLOTS')" @click="quickView(product, context)">
										<i class="fa fa-calendar"></i> <span class="button-text">{{ 'COM_SELLACIOUS_PRODUCTS_DELIVERY_SLOTS' | t }}</span>
									</a>
								</template>
							</template>
						</template>
					<label class="product-compare ctech-btn ctech-btn-primary ctech-text-white hasTooltip" :title="t('COM_SELLACIOUS_PRODUCTS_COMPARE')" v-if="product.compare_allow && params.compare_pages.includes('products') && mod_params.displaycomparebtn == '1'">
						<i class="fa fa-balance-scale"></i> <span class="button-text">{{ 'COM_SELLACIOUS_PRODUCTS_COMPARE' | t }}</span>
						<input type="checkbox" class="btn-compare" :data-item="product.code"/>
					</label>
					<a class="ctech-btn ctech-btn-primary ctech-text-white btn-quick-view hasTooltip" :title="t('COM_SELLACIOUS_PRODUCTS_QUICK_VIEW')" @click="quickView(product, context)"
					   v-if="params.show_modal.includes('products') && mod_params.displayquickviewbtn == '1'">
						<i class="fa fa-eye"></i> <span class="button-text">{{ 'COM_SELLACIOUS_PRODUCTS_QUICK_VIEW' | t }}</span>
					</a>
					</div>
				</template>
				<template v-if="params.cart_pages.includes('products') || params.buynow_pages.includes('products')">
					<div class="product-cart-buttons">
						<component v-if="params.login_to_see_price == '0' || (params.login_to_see_price == '1' && !user.guest)" :is="`product-addtocart-${product.pricing_type}`" :product="product" :params="params" :mod_params="mod_params" :user="user" variant="primary" :show_nostock="true"></component>
						<component v-if="params.login_to_see_price == '0' || (params.login_to_see_price == '1' && !user.guest)" :is="`product-buynow-${product.pricing_type}`" :product="product" :params="params" :mod_params="mod_params" :user="user" variant="dark" :show_nostock="false"></component>
					</div>
				</template>
